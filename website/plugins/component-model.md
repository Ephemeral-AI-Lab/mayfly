...
插件向 Mayfly service 提交 renderer-neutral definition，definition 的
`render()` 返回 canonical `MayflyUiNode`。
...
插件拥有领域状态与 definition；Mayfly registry 拥有当前 registration；core
...
- 交互通过 surface 的 `onEvent(event, context)`，context 提供 AbortSignal 与
  revision；
- 状态变化后调用 registration 的 `set(node)`；
