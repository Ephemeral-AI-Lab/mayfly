...
Mayfly 不创建子插件系统。外部插件、Mayfly 官方行与 dsh 原生 service 都是同一棵
...
dsh 已有的能力直接复用；Mayfly 不复制 command、tool、projection、setting 或
Agent API。只有终端 UI 需要 Mayfly seam。

需要当前 Agent 时 inject `mayflyCurrentAgent`：
...
UI contribution 返回 `MayflyUiNode`。插件不 import pi-tui、不拼 ANSI、不读取
