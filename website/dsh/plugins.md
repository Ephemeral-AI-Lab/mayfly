...
- **terminal 工具** → Mayfly 渲染**专属终端卡**（`$ command` + exit 徽章 + 输出截断，见[流式会话与工具卡片](/features/streaming)）；
- **lsp 工具** → 通用工具卡（暂无专属卡面）；
- **Code Mode**：PTC 模式在 Mayfly 中照常切换，但模型实际能用 `run_code` 之前，profile 必须装运行时——`--dump-config` 里没有 code-runtime 行就是没装；
