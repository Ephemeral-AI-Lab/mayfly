...
- MCP 工具注册进 `ctx.tools` 后，在 Mayfly 会话里就是普通工具调用——按[通用工具卡](/features/streaming)呈现（Mayfly 当前无 MCP 专属卡面）；
- 满足上述图片条件时，MCP 返回的图片块可在 Mayfly 会话里渲染（Mayfly 的 `mayfly-attachments` 就是 `ctx.attachments` 的实现方）；
