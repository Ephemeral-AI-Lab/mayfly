...
::: tip 与 Mayfly 的关系
权限预设（`dsh-permission-presets`）**在默认装配里**（dsh-base 84 行之一）。Mayfly 目前以审批面板承担 `ask` 策略；预设切换器已落地——在输入框直接敲 `/permission`（裸调用由输入层拦截，带参透传上游命令）即可打开预设切换面板，持久默认由 `/settings` 面板的 `permission.defaultPreset` 行维护。
