...
| Theme | `/theme` (session-level); `/settings` or the file for the persisted default | the `mayfly:` section of `settings.yaml` ([Theming](/en/guide/theme)) |
| Mayfly preferences (update check, fold defaults, …) | `/settings` | the `mayfly:` section of `settings.yaml` |
...
`transcriptView` follows the Harness Chat work-details modes (see [Streaming transcript](/en/features/streaming#work-details)). Compact, Standard, and Detailed fold a completed turn behind its `Took 38s` header, keeping the final answer visible; process work between replies collapses into one titled group (Standard also names the running command, path, query, or reasoning; Detailed keeps the running turn open). Verbose keeps every card open. Ctrl+O expands the most recent `expandTurns` turns. The old per-family `transcript` settings and `recentStepsRetention` are no longer interpreted.
...
`/theme dark|light|ocean|paper|auto` switches instantly; `/theme custom <path>` mounts a custom JSON palette — hot switches never lose your draft. `/theme` is the session-level switch; the persisted default theme is set through the `/settings` panel or `mayfly.theme` in settings.yaml (applied at startup). The full semantic token table and the custom file format are in [Theming](/en/guide/theme).
...
| `DSH_MAYFLY_ATTACHMENT_DIR` | Mayfly attachment storage | `$DSH_HOME/attachments/` |
