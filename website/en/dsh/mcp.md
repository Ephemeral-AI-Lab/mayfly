...
- Once registered on `ctx.tools`, MCP tools are ordinary tool calls in a Mayfly session — rendered as the [generic tool card](/en/features/streaming) (Mayfly has no MCP-specific card yet);
- under the image conditions above, MCP image blocks render in a Mayfly session (Mayfly's `mayfly-attachments` is the `ctx.attachments` implementation);
