...
- **terminal tools** → Mayfly renders the dedicated **terminal card** (`$ command` + exit badge + capped output, see [Streaming transcript & tool cards](/en/features/streaming));
- **lsp tool** → the generic tool card (no dedicated card yet);
- **Code Mode**: PTC mode switches in Mayfly as usual, but the profile must mount a runtime before the model can actually use `run_code` — no code-runtime row in `--dump-config` means it isn't installed;
