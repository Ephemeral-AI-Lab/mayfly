...
| Policy | Semantics |
| --- | --- |
| `ask` (default) | requests go to the chain of answerers — **Mayfly's four-choice approval panel is an answerer** (see [Approvals & questionnaires](/en/features/approval)); with no answerer, fail closed |
...
::: tip Relation to Mayfly
Permission presets (`dsh-permission-presets`) **are in the default assembly** (one of dsh-base's 78 rows). Mayfly currently services the `ask` policy through its approval panel; the preset switcher has shipped — typing a bare `/permission` in the input box (intercepted by the input layer; an argumented line passes through to the host command) opens the preset panel, and the persisted default is maintained through the `/settings` panel's `permission.defaultPreset` row.
Mayfly's yolo badge identifies the current `danger-full-access` + `never` permission preset. `Shift+Tab` toggles only plan state through `/plan` and `/plan off`; `/permission` controls YOLO independently. Plan and YOLO can be active together, and the status bar shows both.
:::
