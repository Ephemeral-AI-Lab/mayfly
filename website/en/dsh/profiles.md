...
A profile is a **named assembly stored in the Harness home**: it lists the bundles it layers, holds its own `cordis.patch.yml` override, and owns its independently installed plugins. Booting `dsh --profile mayfly` composes layers in this order:
...
A patch locates an entry by id and either replaces its entire config or inserts a new entry — **upper layers always overwrite lower ones**, which is the mechanical guarantee that every surface is customizable. The distribution ships `web` and `headless` as template profiles; installing Mayfly means placing the Mayfly bundle into your own profile (see [Quickstart](/en/guide/)).

The most direct way to inspect the composition:

```sh
dsh --profile mayfly --dump-config     # print the actual assembled plugin tree
```
...
| `$DSH_HOME/attachments/` | the attachment store (Mayfly's default location; relocatable via `DSH_MAYFLY_ATTACHMENT_DIR`) |

## A profile is a pnpm workspace

`dsh plugin --profile <name> add <pkg>` forwards verbatim to pnpm inside the profile directory — so:

- local checkouts install via the `link:` protocol during development (Mayfly's [contributor guide](/en/plugins/contributing) links the script's authoritative list — the release closure plus selected validation adapters — exactly this way);
