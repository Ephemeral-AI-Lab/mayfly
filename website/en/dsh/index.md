...
::: info Version basis
This handbook tracks Harness npm release `0.1.7-alpha.2` and Mayfly `0.1.0-alpha.5`. When in doubt, trust your installed `dsh --version` and `--dump-config`.
:::

## What dsh is

dsh is a **plugin-based agent harness**: model adapters, tool execution, session persistence, approval and sandbox policies, settings, and credentials are all assembled as [Cordis](https://www.npmjs.com/package/@deepseek-ai/cordis) plugins. Every interface you see — Mayfly's TUI, the browser web app, the one-shot headless runner — is just a different assembly of the same plugin tree.
