...
- **`/btw` / `/agents`** — share one auxiliary slot. A live BTW/continuable child reuses the complete main layout and editor; a one-shot or cold child uses the full-fidelity readonly transcript panel. `F7` toggles main/auxiliary and `F8` closes it. `/agents stop <id>` accepts only continuable children, while browser stops additionally require typed-`y` confirmation.
