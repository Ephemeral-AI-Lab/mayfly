...
| Form panel | non-wrapping `Up` / `Down` changes fields in navigation; typing or first `Enter` enters text editing, then `Enter` confirms while invalid input stays on the field; `Alt+Enter` inserts a textarea newline; a select uses `Enter` to enter, `←` / `→` to adjust, and `Enter` to apply; Tab discards an unconfirmed select adjustment and moves to the next semantic group, while `Escape` climbs layers |
