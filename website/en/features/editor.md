...
With the editor focused, a contextual key chain applies (see [Key bindings](/en/reference/keys)): Escape first dismisses the completion popup, then clears the draft, then safely retracts the just-submitted message (falling back to interruption when retraction is unavailable). Ctrl-C never retracts: it always interrupts current work first and preserves a next-message draft; only when there is nothing to interrupt does it clear an idle draft or enter the double-press-within-1-second exit path. Ctrl-S steers the non-empty draft into the current turn. ↑/↓ always belong to editor history (the queue pane only lists pending messages — it never takes those keys), while PageUp/PageDown scroll the current transcript. Live BTW/subagents reuse this editor chain; `F7` toggles main/auxiliary and `F8` closes the auxiliary slot. When a one-shot or cold child replaces the editor slot with the readonly transcript panel, that panel owns scrolling and Escape-to-close.
