...
::: info 当前发布线
`v0.1.0-alpha.5` 是当前预览版，并发布到 npm **`latest`**。普通安装不写 dist-tag；CI 和可复现环境应锁定 Mayfly `0.1.0-alpha.5` 与 Harness `0.1.7-rc.1`。本页是用户安装路径；贡献者的本地开发安装见[贡献本仓库](/plugins/contributing)。
:::
...
| dsh CLI | 推荐的 `mayfly` 启动器已内含精确版本 `0.1.7-rc.1`；只有下方“dsh 直装”路径需要单独安装 |
