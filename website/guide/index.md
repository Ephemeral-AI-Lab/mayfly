...
::: info 当前发布线
`v0.1.0-alpha.5` 是当前预览版，并发布到 npm **`latest`**。普通安装不写 dist-tag；CI 和可复现环境应锁定 Mayfly `0.1.0-alpha.5` 与 Harness `0.1.7-alpha.2`。本页是用户安装路径；贡献者的本地开发安装见[贡献本仓库](/plugins/contributing)。
:::
...
| dsh CLI | 推荐的 `mayfly` 启动器已内含精确版本 `0.1.7-alpha.2`；只有下方“dsh 直装”路径需要单独安装 |

## 安装（预览版）

**推荐：一体化 `mayfly` 启动器。** 它把固定的 Harness 闭包封装为公共层和平台层归档，npm 安装时只处理一个无依赖包，不解析 Harness 图，也不运行其中的安装脚本：
...
壳包的 npm 安装只写入启动器与压缩运行时层。第一个需要 dsh 的命令只会把公共层和当前平台层以有界内存展开到用户缓存，不再联网；首次运行 `mayfly` 仍会经 `dsh plugin add` 和 pnpm 装配 Mayfly profile，这个独立操作下载 Mayfly 插件闭包，并可从 pnpm 缓存续传。国内网络建议同时配置两个 registry：
...
- 无版本后缀会解析 npm `latest`；CI、插件清单与可复现环境应使用上方的精确版本。
- 升级到新版：壳包用户重跑 `npm -g install @ephemeral-ai/mayfly-cli`（重装即升级——壳按自身版本校准 profile 里的 Mayfly，宿主线随之固定）；dsh 直装用户在 Mayfly 中输入 `/update`（应用内安全升级：预检、快照、装机冒烟、失败自动回滚），或重跑同一条 `plugin add`。
...
```sh
mayfly                        # 交互模式：欢迎横幅 + 输入编辑器
mayfly 修复登录页的空指针       # 直接执行任务
# dsh 直装用户：dsh --profile mayfly（两条等价，壳包只是把宿主与 profile 管了起来）
```
...
<p align="center">
  <img src="/shots/app-conversation.svg" width="720" alt="Mayfly 终端界面：实时会话、工具卡片、编辑器与状态栏">
</p>
