...
bundle，针对 Harness `0.1.7-rc.1` 构建。Mayfly `0.1.0-alpha.9`
...
流式思考时，思考块标题显示已用时间，例如 `✻ 思考中 · 6s`；编辑器上方的
activity 行是唯一的动画 spinner，也是唯一显示当前阶段估算 token 数与输出速率的位置。思考结束后折叠为一行 `✻ 已思考 6s`。速率按每四个字符一个 token 估算，不计首个 chunk
