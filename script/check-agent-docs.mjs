...
export function auditAgentDocs(root, options = {}) {
  ...
  const harnessSource = join(root, 'packages', 'mayfly', 'src', 'interaction', 'session-commands.ts')
  ...
  if (existsSync(projectSkills) && walk(projectSkills, 'SKILL.md').length > 0) {
    problems.push('.agents/skills must stay empty; Mayfly maintainer guidance belongs in AGENTS.md and deterministic checks')
  }
  if (existsSync(join(root, 'docs', 'skills'))) problems.push('docs/skills is a retired mirror of removed project skills')
  if (existsSync(join(root, 'docs', 'mayfly-skills-plan.md'))) problems.push('docs/mayfly-skills-plan.md must live under docs/history after skill consolidation')
  ...
  for (const file of agentFiles) {
    ...
    if (/\b\d+ Mayfly-owned rows\b/u.test(source)) problems.push(`${file.slice(root.length + 1)} duplicates the bundle row count`)
  ...
  if (checkPreset) {
    const presetRoot = join(root, 'packages', 'mayfly', 'presets', 'mayfly-cordis')
    const skillRoot = join(presetRoot, 'skills')
    const expected = ['cordis-plugin-development', 'editing-cordis-compositions', 'mayfly-plugin-development']
    const actual = readdirSync(skillRoot, { withFileTypes: true }).filter(entry => entry.isDirectory()).map(entry => entry.name).sort()
    if (JSON.stringify(actual) !== JSON.stringify(expected)) problems.push(`mayfly-cordis skills differ: ${actual.join(', ')}`)
    ...
    if (!composition.includes('Do not load either plugin-development skill')) {
      problems.push('mayfly-cordis does not exclude ordinary and Mayfly-repository work from plugin skills')
    }
  }
  return problems
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(`agent docs: ${walk(ROOT, 'AGENTS.md').length} instruction files and Mayfly preset skills are consistent`)
}
