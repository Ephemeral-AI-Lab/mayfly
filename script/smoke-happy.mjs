// Real-process smoke (D48, in CI): boots the shipped Mayfly plugin tree
...
// D48 exit clamp's mayfly-overflow.log stays empty (a clamped row is still a
...
const profile = process.env.SMOKE_PROFILE ?? 'mayfly-smoke'
...
const PATHOLOGICAL = [
  ...
  '/home/x/dev/deepseek-harness-plugin/mayfly/mayfly/.claude/worktrees/a-rather-deep-checkout-name/src',
].join('\n')
...
const overflowLog = join(piAgent, 'mayfly-overflow.log')
...
if (!ok) {
  ...
  if (overflow.trim() !== '') {
    console.error('--- mayfly-overflow.log (clamped rows are component bugs) ---')
