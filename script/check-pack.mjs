...
const outDir = resolve(process.env.MAYFLY_PACK_DIR ?? join(ROOT, '.artifacts', 'pack'))
...
function validateManifest(name, manifest, root, relativeDir) {
  ...
  if (manifest.homepage !== HOMEPAGE_URL) fail(`${name}: packed homepage must equal ${HOMEPAGE_URL}`)
  if (!isDeepStrictEqual(manifest.bugs, { url: BUGS_URL })) fail(`${name}: packed bugs URL must equal ${BUGS_URL}`)
  if (!isDeepStrictEqual(manifest.engines, { node: NODE_RANGE })) fail(`${name}: packed Node engine must equal ${NODE_RANGE}`)
  for (const tableName of ['dependencies', 'peerDependencies', 'optionalDependencies', 'devDependencies']) {
    for (const [dependency, spec] of Object.entries(manifest[tableName] ?? {})) {
      ...
      if (dependency.startsWith('@ephemeral-ai/') && spec !== manifest.version) {
...
function verifyExternalUiKit(uiTarball) {
  if (uiTarball === undefined) {
    fail('external UI kit fixture requires the packed UI tarball')
    return
  }
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'mayfly-ui-kit-pack-'))
  ...
  try {
    ...
    writeFileSync(join(kitRoot, 'package.json'), `${JSON.stringify({
      name: '@mayfly-pack-fixture/user-kit',
      version,
      type: 'module',
      main: 'lib/index.js',
      ...
      files: ['lib/**/*'],
      peerDependencies: { '@ephemeral-ai/mayfly-ui': version },
    }, null, 2)}\n`)
    writeFileSync(join(kitRoot, 'lib', 'index.js'), `
import { defineMayflyComponent, ui } from '@ephemeral-ai/mayfly-ui'
export const metric = defineMayflyComponent({
  id: '@mayfly-pack-fixture/metric',
...
import type { MayflyComponentFactory } from '@ephemeral-ai/mayfly-ui'
export interface MetricProps { readonly label: string, readonly value: number }
export declare const metric: MayflyComponentFactory<MetricProps>
`)
    ...
    writeFileSync(join(consumerRoot, 'package.json'), `${JSON.stringify({
      ...
      type: 'module',
      dependencies: {
        '@ephemeral-ai/mayfly-ui': `file:${uiTarball}`,
        '@mayfly-pack-fixture/user-kit': `file:${kitTarball}`,
      },
    }, null, 2)}\n`)
...
import { metric } from '@mayfly-pack-fixture/user-kit'
...
import { metric, type MetricProps } from '@mayfly-pack-fixture/user-kit'
import type { MayflyUiNode } from '@ephemeral-ai/mayfly-ui'
const props: MetricProps = { label: 'Context', value: 42 }
const node: MayflyUiNode = metric.render(props)
void node
`)
...
for (const relativeDir of PACKAGE_DIRS) {
  ...
  if (sourceManifest.name === '@ephemeral-ai/mayfly-cli') {
  ...
  if (sourceManifest.name !== '@ephemeral-ai/mayfly-cli') {
    ...
    if (sourceManifest.name === '@ephemeral-ai/mayfly') attwArgs.push('--exclude-entrypoints', './cordis.patch.yml')
...
// The consolidated runtime and UI declarations currently occupy 192 files and
// about 1.795 MB; retain about 3% headroom while still catching accidental
// entry/chunk growth.
if (libraryFiles > 222) fail(`library lib output has ${libraryFiles} files; budget is 222`)
if (libraryBytes > 1_850_000) fail(`library lib output has ${libraryBytes} bytes; budget is 1850000`)
