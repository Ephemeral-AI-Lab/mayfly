...
Definition of done: packages/mayfly/tests/transcript/version.spec.ts passes. That spec is the completeness proof — it asserts that every exact-pinned @deepseek-ai/dsh-* dependency and devDependency across all workspace package manifests, every ^-ranged dsh peer, every pnpm-workspace.yaml minimumReleaseAgeExclude entry, and the HARNESS_LINE constant in packages/mayfly/src/interaction/session-commands.ts all agree on one line. Verify with: pnpm vitest run packages/mayfly/tests/transcript/version.spec.ts
