 ...
 * (`packages/mayfly/lib/core.js`) with the real dark theme palette
 * (`packages/mayfly/lib/theme-dark.js`), renders it to ANSI rows, asserts the
...
const coreLibUrl = new URL('../../packages/mayfly/lib/core.js', import.meta.url)
...
const { CURSOR_MARKER, truncateToWidth, visibleWidth, wrapTextWithAnsi } =
  await import(new URL('../../packages/mayfly/node_modules/@earendil-works/pi-tui/dist/index.js', import.meta.url).href)
...
const { renderLayoutFrame } =
  await import(new URL('../../packages/mayfly/node_modules/@earendil-works/pi-tui/dist/layout.js', import.meta.url).href)

/** Minimal deterministic MayflyEditor fake, mirrored from core's ui-compiler spec. */
