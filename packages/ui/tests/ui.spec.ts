...
describe('ui builders', () => {
  it('builds and freezes every content leaf without changing its wire shape', () => {
    const leaves = [
      ...
      ui.fields([{ label: 'branch', value: [{ text: 'main', styles: ['strong'] }] }]),
    ...
    expect(leaves).toEqual([
      ...
      { kind: 'fields', rows: [{ label: 'branch', value: [{ text: 'main', styles: ['strong'] }] }] },
      ...
      { kind: 'markdown', source: '| A | B |\n| - | - |' },
      { kind: 'diagram', diagram: 'mermaid', source: 'graph LR; A --> B' },
...
describe('defineMayflyComponent', () => {
  it('returns frozen metadata and deeply freezes each render result', () => {
    const component = defineMayflyComponent<{ readonly label: string }>({
  ...
  it('validates only definition metadata and leaves schema admission to core', () => {
    ...
    const component = defineMayflyComponent({
      id: '@acme/future-node',
  ...
  it('rejects cyclic component output', () => {
    ...
    output.self = output
    const component = defineMayflyComponent({ id: '@acme/cycle', render: () => output as never })
  ...
  it('rejects invalid definitions, ids, and render functions', () => {
    expect(() => defineMayflyComponent(null as never)).toThrow('definition must be an object')
    expect(() => defineMayflyComponent({ id: 'Bad ID', render: () => ui.text('x') })).toThrow('component id')
    expect(() => defineMayflyComponent({ id: '@acme/kit', render: null as never })).toThrow('render must be a function')
  })
})
