...
The `mayfly` launcher shell (S37, decision D50④): a standalone global package,
...
`mayfly` profile, and execs the pinned host with translated arguments.

Scope boundaries:

- `--profile` is swallowed and always `mayfly`; `plugin` gets the flag after
  the subcommand word (the host's own usage rule); `-V` is self-answered
  (shell · Mayfly pin · harness line). No other surface exists — the shell
  never parses Mayfly arguments (startup.ts owns that).
- Calibration skips `link:`/`file:` lanes (the three-lane rule: `mayfly` is
  npm-only; `mayfly-dev`/`mayfly-<tag>` are link lanes and must never be
  ...
  18 min for 455 packages). No `mayfly upgrade` exists by ruling —
  reinstalling the shell is the upgrade.
- The `MAYFLY_LAUNCHER=mayfly` child env is the branding seam. `MAYFLY_DSH_BIN`
  exports the materialized nested host path so in-app `/plugin install`
  delegates to the exact same pinned dsh runtime.
- Creative mode belongs entirely to `@ephemeral-ai/mayfly`; the launcher carries
  no preset payload and performs no host-installation writes. This keeps
  `mayfly` and direct `dsh --profile` launches on the same bundle path.
- The updater family (`packages/mayfly/src/interaction/updater/`, D52) stays the
...
`src/runtime.ts` extracts the common archive plus only the current native layer below `$DSH_HOME/cache/mayfly-cli-runtime/<mayfly>-<dsh>-<platform>-<arch>` through the bundled `tar` implementation. Extraction is synchronous with 1 MiB read blocks: async `tar` path reservations retained the 28k-entry closure until completion and measured near 500 MB RSS, while the bounded synchronous path stays streaming and measured near 160 MB. The prepared tree and cache hits validate the host entry and native file sizes against the pack-generated `node_modules/.mayfly-runtime.json`; checks remain bounded to at most 64 sentinels instead of scanning the dependency tree. Damaged caches are replaced only after a complete prepared tree exists, by quarantining the invalid target and atomically publishing the replacement; concurrent winners are reused. Old cache-key directories are left alone. Version output and locally handled plugin-market commands never extract the payload.
