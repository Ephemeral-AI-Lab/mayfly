...
function fixtureHome(files: Record<string, string>): { home: string, root: string } {
  const home = mkdtempTracked('mayfly-cli-calibrate-')
...
function profileManifest(spec: string): string {
  return JSON.stringify({ name: 'mayfly-profile', dependencies: { '@ephemeral-ai/mayfly': spec } })
}

/** The installed-bundle manifest. */
function installedManifest(version: string): string {
  return JSON.stringify({ name: '@ephemeral-ai/mayfly', version })
}
...
describe('calibrate', () => {
  it('passes through with zero spawns when the installed bundle matches the pin', async () => {
    ...
    fixtureHome({
      ...
      'node_modules/@ephemeral-ai/mayfly/package.json': installedManifest(PIN),
    })
  ...
  it('skips a link/file dev lane without touching the profile', async () => {
    let spawned = false
    fixtureHome({ 'package.json': profileManifest('link:/checkout/packages/mayfly') })
    ...
    await expect(calibrate({ version: PIN, dshBinJs: 'nested-dsh.js' })).resolves.toEqual({
      action: 'link-lane',
      spec: 'link:/checkout/packages/mayfly',
    })
  ...
  it('installs the pin through the nested host and verifies the result', async () => {
    ...
    cliInternals.spawnOnce = withProbe(async (cmd, args, opts) => {
      ...
      writeFileSync(join(root, 'node_modules', '@ephemeral-ai', 'mayfly', 'package.json'), installedManifest(PIN))
      return OK
    })
    await expect(calibrate({ version: PIN, dshBinJs: 'nested-dsh.js' })).resolves.toEqual({ action: 'installed' })
    expect(calls).toEqual([{
      cmd: process.execPath,
      args: ['nested-dsh.js', 'plugin', '--profile', 'mayfly', 'add', `@ephemeral-ai/mayfly@${PIN}`],
  ...
  it('never downgrades a profile that /update advanced past the shell', async () => {
    let spawned = false
    fixtureHome({ 'node_modules/@ephemeral-ai/mayfly/package.json': installedManifest('0.1.0-rc.6') })
    cliInternals.spawnOnce = async () => {
      spawned = true
      return OK
    }
  ...
  it('retries once with -w when pnpm refuses a workspace-root write', async () => {
    ...
    expect(calls.map(call => call.args.join(' '))).toEqual([
      `nested-dsh.js plugin --profile mayfly add @ephemeral-ai/mayfly@${PIN}`,
      `nested-dsh.js plugin --profile mayfly add -w @ephemeral-ai/mayfly@${PIN}`,
    ])
  })
  ...
  it('classifies the win32 shell-not-found exit 9009 after a failed install as pnpm-missing, keeping the dsh line as detail', async () => {
    ...
    cliInternals.spawnOnce = withProbe(async () => ({
      ...
      stderr: 'dsh: pnpm failed in profile directory C:\\Users\\x\\.dsh\\profiles\\mayfly\n',
      timedOut: false,
    }))
    await expect(calibrate({ version: PIN, dshBinJs: 'nested-dsh.js' })).resolves.toEqual({
      action: 'failed',
      reason: 'pnpm is missing on PATH — npm i -g pnpm@11 (or: corepack enable pnpm@11)',
      kind: 'pnpm-missing',
      detail: ['dsh: pnpm failed in profile directory C:\\Users\\x\\.dsh\\profiles\\mayfly'],
    })
  })
  ...
  it('reports a timed-out install with the timeout class and the marker tail', async () => {
    ...
    await expect(calibrate({ version: PIN, dshBinJs: 'nested-dsh.js' })).resolves.toEqual({
      ...
      kind: 'timeout',
      detail: ['pnpm: downloading…', 'mayfly: install timed out'],
    })
  })
  ...
  it('fails on a spawn error and on a post-install mismatch', async () => {
    ...
    await expect(calibrate({ version: PIN, dshBinJs: 'nested-dsh.js' })).resolves.toEqual({
      action: 'failed',
      reason: 'profile reports @ephemeral-ai/mayfly@uninstalled after install',
      kind: 'verify',
    })
  })

  it('reads broken manifests as absent and installs through them', async () => {
    fixtureHome({ 'package.json': '{ broken', 'node_modules/@ephemeral-ai/mayfly/package.json': '{ also broken' })
    cliInternals.spawnOnce = withProbe(async () => OK)
    await expect(calibrate({ version: PIN, dshBinJs: 'nested-dsh.js' })).resolves.toEqual({
      action: 'failed',
      reason: 'profile reports @ephemeral-ai/mayfly@uninstalled after install',
  ...
  it('reads a non-string installed version as absent', async () => {
    fixtureHome({ 'package.json': JSON.stringify({ dependencies: {} }), 'node_modules/@ephemeral-ai/mayfly/package.json': JSON.stringify({ version: 3 }) })
    cliInternals.spawnOnce = withProbe(async () => OK)
    await expect(calibrate({ version: PIN, dshBinJs: 'nested-dsh.js' })).resolves.toEqual({
      action: 'failed',
      reason: 'profile reports @ephemeral-ai/mayfly@uninstalled after install',
