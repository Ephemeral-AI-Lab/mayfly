 ...
 * bundle version (the version.spec lockstep), and the `mayfly` profile
 ...
 * `dsh plugin --profile mayfly add @ephemeral-ai/mayfly@<pin>` away. A
 * `link:`/`file:` spec is the dev lane by the three-lane rule
 * (fe8512c): calibration never touches it. There is no `mayfly upgrade`
 * — upgrading means reinstalling the shell, which moves the pin.
 *
 * @module @ephemeral-ai/mayfly-cli/calibrate
 */
...
/** What calibration needs: the Mayfly pin and nested host entry. */
export interface CalibrateOptions {
  /** The pinned Mayfly bundle version (the shell's own manifest version). */
...
export function mayflyProfileRoot(): string {
  return join(dshHome(), 'profiles', PROFILE)
}

/**
 * Calibrate the `mayfly` profile to the pin. Installs run behind a pnpm
...
export async function calibrate(options: CalibrateOptions): Promise<CalibrationOutcome> {
  ...
  const runInstall = (extra: readonly string[]) => cliInternals.spawnOnce(cliInternals.execPath, [
    options.dshBinJs, 'plugin', '--profile', PROFILE, 'add', ...extra, `@ephemeral-ai/mayfly@${options.version}`,
  ...
  if (installedBundleVersion(root) !== options.version) {
    return { action: 'failed', kind: 'verify', reason: `profile reports @ephemeral-ai/mayfly@${installedBundleVersion(root) ?? 'uninstalled'} after install` }
  }
...
/** The profile's `@ephemeral-ai/mayfly` dependency spec, when its manifest names one. */
function bundleSpec(root: string): string | undefined {
  ...
  try {
    ...
    for (const block of [manifest.dependencies, manifest.devDependencies]) {
      const spec = block?.['@ephemeral-ai/mayfly']
...
function installedBundleVersion(root: string): string | undefined {
  const text = cliInternals.readTextFile(join(root, 'node_modules', '@ephemeral-ai', 'mayfly', 'package.json'))
  if (text === undefined) return undefined
  try {
