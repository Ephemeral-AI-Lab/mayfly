...
description: >-
  ...
  mayfly-cordis session, including additive Mayfly UI prototypes. Use for inspect,
...
9. After acceptance, ask whether to keep a local package, create a GitHub
   ...
   `mayfly-plugin-development` only after that choice.
...
```json
{
  ...
  "code": {
    "host": "return { name: 'probe', inject: ['mayflyPanes'], apply(ctx) { /* work */ } }"
  }
}
```
...
Use Mayfly services only for terminal UI:
...
Use `mayflyCurrentAgent.current()` when a native Agent-scoped service needs the
Agent selected by Mayfly. Do not guess service methods or depend on core
renderer objects such as `mayflyScreen`, `mayflyComponents`, raw focus, pi-tui,
ANSI, or terminal width.

## Direct Mayfly UI example
...
return {
  ...
  apply(ctx) {
    ctx.commands.register({
      ...
      handler() {
        ctx.mayflyOverlays.close('probe.health')
...
Renderer-neutral snapshots contain data, not pi-tui components. Build them in
event/projection callbacks and publish with `set()`; rendering performs no I/O.
