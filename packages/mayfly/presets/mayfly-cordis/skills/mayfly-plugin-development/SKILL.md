---
name: mayfly-plugin-development
description: Create, extend, or migrate a durable ordinary Cordis plugin that uses native dsh services and optionally contributes Mayfly terminal UI. Use only after the user chooses persistence. Not for ephemeral prototypes, Mayfly repository maintenance, agent presets, or publishing without explicit authorization.
---

# Develop a durable Mayfly plugin
...
A Mayfly plugin is an ordinary Cordis plugin. It uses native dsh services
directly and adds Mayfly services only for terminal UI.
...
- terminal UI: `mayflyPanes`, `mayflyStatus`, `mayflyOverlays`, or
  `mayflyEditorExtensions`;
- current frontend Agent: `mayflyCurrentAgent`, only when an Agent-scoped
...
```json
{
  ...
  "peerDependencies": {
    ...
    "@ephemeral-ai/mayfly-ui": "^0.1.0-alpha.1"
  }
}
```
...
There is no additional Mayfly metadata file or authoring CLI.
...
Snapshots are immutable data and are published with `set()`. Mayfly owns terminal
...
`mayflyCurrentAgent.current()` at operation time or subscribe and clear derived
state on revision.
...
```sh
dsh plugin --profile mayfly-my-plugin add file:/absolute/path/to/my-plugin
dsh --profile mayfly-my-plugin
```
...
and restart. Never mutate the production `mayfly` profile for acceptance.
