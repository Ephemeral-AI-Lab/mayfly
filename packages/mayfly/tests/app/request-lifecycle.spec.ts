 ...
 *
 * @module @ephemeral-ai/mayfly/app/tests/request-lifecycle
 */

import { describe, expect, it } from 'vitest'
import { Context } from '@deepseek-ai/cordis'
import { createMayflyRequestController } from '../../src/app/request-lifecycle.ts'

describe('Mayfly request lifecycle', () => {
  it('emits started and interrupted exactly once, then rejects stale transitions', () => {
    const ctx = new Context()
    const events: unknown[] = []
    ctx.on('mayfly/request-state-changed', event => { events.push(event) })
