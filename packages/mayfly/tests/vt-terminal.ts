 ...
 * helpers, so `bootMayfly({ terminal: new VtTerminal(...) })` boots the same
...
import { shortenCwd } from '../src/transcript/status-cwd.ts'
import { shortenHome } from '../src/transcript/banner.ts'
...
export class VtTerminal extends FakeTerminal {
   ...
   * trimmed right) joined by newlines. Refuses the alternate buffer — Mayfly
  ...
  async frame(normalize?: (row: string) => string): Promise<string> {
    ...
    if (buffer.type !== 'normal') throw new Error('VT entered the alternate buffer — Mayfly renders on the main screen')
    const rows: string[] = []
  ...
  /**
   * The underlying headless terminal for the app-shot SVG painter.
   * `paintTerminalSvg` reads viewport rows only (`baseY + row`), so exposing
   * the live buffer never leaks scrollback into a frame.
   */
  get painterTerminal(): Terminal {
    return this.vt
  }
}
