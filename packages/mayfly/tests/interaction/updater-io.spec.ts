...
import { cleanOutput, updaterInternals } from '../../src/interaction/updater/io.ts'
...
describe('updater/io defaultSpawnOnce', () => {
  ...
  it('layers explicit env entries over the process environment', async () => {
    const outcome = await updaterInternals.spawnOnce(NODE, ['-e', 'process.stdout.write(process.env.MAYFLY_IO_PROBE ?? "unset")'], {
      env: { MAYFLY_IO_PROBE: 'marked' },
    })
...
describe('updater/io fs defaults', () => {
  const dir = mkdtempTracked('mayfly-updater-io-')
