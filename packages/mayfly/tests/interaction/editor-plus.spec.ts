/**
 * Tests for the `mayfly-editor-plus` plugin: prompt/bash mode switching, shell
...
import type { MayflyAutocompleteProvider, MayflyComponent } from '../../src/core/index.ts'
...
function echoes(screen: FakeScreen): MayflyComponent[] {
  return screen.children.filter(child => !(child instanceof FakeMayflyEditor))
    .filter(child => child.render(80)[0]?.startsWith('$$ ') === true)
}

describe('mayfly-editor-plus input modes', () => {
  ...
  it('keeps the shell hue over the slash-context resolution while bash is active', async () => {
    const { editor } = await mount()
    type(editor, '!')
    // A leading `/` is a path separator in bash mode; mayfly-input's
  ...
  it('keeps bash mode when Escape clears a non-empty buffer', async () => {
    ...
    type(editor, '!')
    type(editor, 'ls')
    // Escape clears the draft through mayfly-input's chain; the mode stays.
  ...
  it('executes a bash submission, echoes it, records history, and reverts to prompt mode', async () => {
    const { screen, editor, followup, inputFiber } = await mount()
    ...
    const echo = echoes(screen)[0] as MayflyComponent
  ...
  it('runs the default executor through the real shell', async () => {
    const { screen, editor } = await mount()
    type(editor, '!')
    type(editor, 'echo mayfly-plus-real')
    editor.handleInput(KEY.enter)
    await vi.waitFor(() => {
      expect(echoes(screen)).toHaveLength(1)
    })
    expect(echoes(screen)[0]?.render(80)).toEqual(['$$ $echo mayfly-plus-real', '_mayfly-plus-real_'])
  })
...
describe('mayfly-editor-plus attach lifecycle', () => {
  it('attaches when mayfly-input mounts after it', async () => {
  ...
  it('re-attaches to the new editor when mayfly-input remounts (theme reload)', async () => {
...
describe('mayfly-editor-plus slash completion', () => {
  async function providerOf(options: { withAgent?: boolean } = {}): Promise<{
    provider: MayflyAutocompleteProvider
    ctx: Context
    editor: FakeMayflyEditor
  }> {
    const mounted = await mount(options)
    const provider = (mounted.editor.autocompleteProvider ?? undefined) as MayflyAutocompleteProvider | undefined
  ...
  it('suggests commands fuzzy-matching the slash prefix, ties keeping registry order', async () => {
    ...
    ctx.commands.register({ name: 'quit', description: 'Exit Mayfly', handler: () => ({ kind: 'success' }) })
  ...
  it('matches aliases behind the canonical name, labeling alias hits with the alias list', async () => {
    ...
    try {
      ...
      expect(suggestions?.items).toEqual([
        ...
        { value: '/quit', label: '/quit (q, exit)', description: 'Exit Mayfly' },
      ])
      ...
      expect(plain?.items.some(item => item.value === '/quit' && item.description === 'Exit Mayfly')).toBe(true)
    } finally {
      clear()
    }
  })
...
describe('mayfly-editor-plus # skill completion', () => {
...
describe('mayfly-editor-plus argument-hint ghost', () => {
  ...
  it('resolves the argument hint through an alias token', async () => {
    ...
    const clear = ctx.mayflyInteractionState.aliases.register('resume', ['r'])
    try {
  ...
  it('never shows the ghost in bash mode and clears it on detach', async () => {
    const { ctx, editor, inputFiber } = await mount()
    ctx.commands.register({
      name: 'btw',
      description: 'Ask a side question',
      input: { hint: '<question>' },
  ...
  it('ghosts a draft restored before the enhancement attaches', async () => {
    // A theme-swap reload order: mayfly-input restores the stashed draft, then
    // editor-plus attaches over it.
    const { ctx, editor, inputFiber, components } = await mount()
    ctx.commands.register({
      name: 'btw',
      description: 'Ask a side question',
      input: { hint: '<question>' },
    ...
    await ctx.plugin(inputPlugin)
    const remounted = components.editors.at(-1)!
    expect(remounted.ghostHint).toBe(' <question>')
  })
})
...
describe('mayfly-editor-plus @ mentions', () => {
  ...
  function fixture(): string {
    const dir = mkdtempTracked('mayfly-plus-files-')
  ...
  it('caps the fs fallback at the suggestion limit', async () => {
    fileMention.setFdProbe(async () => null)
    const { ctx, editor } = await mount()
    const root = mkdtempTracked('mayfly-plus-many-')
