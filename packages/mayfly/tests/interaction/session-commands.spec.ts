...
import type { InfoPanel } from '../../src/interaction/info-panel.ts'
import {
...
} from '../../src/interaction/session-commands.ts'
import { MAYFLY_VERSION } from '../../src/transcript/banner-content.ts'
import { fakeMayflyContext, type FakeScreen } from './fakes.ts'
...
describe('buildVersionSections', () => {
  it('lists the Mayfly and harness release lines', () => {
    const sections = buildVersionSections()
    expect(sections.map(section => section.heading)).toEqual(['Version'])
    expect(sections[0]!.rows).toEqual([
      { label: 'mayfly', segments: [{ text: `v${MAYFLY_VERSION}` }] },
      { label: 'harness', segments: [{ text: '0.1.2-alpha.5' }] },
    ])
  })
  ...
  it('uses a profile-local display identity without changing the release constant', () => {
    ...
    expect(MAYFLY_VERSION).toBe('0.1.0-alpha.1')
  })
})
...
describe('buildChangelogSections', () => {
  it('wraps highlights, marks the current release, and includes known issues', () => {
    ...
    expect(sections.map(section => section.heading)).toEqual([`v${MAYFLY_VERSION} · current`, 'v0.0.0'])
...
describe('buildStatusSections', () => {
  it('lists the session header, counts, agent state, model, and version', () => {
    ...
    expect(sections[1]!.rows[1]!.segments).toEqual([
      { text: `Mayfly v${MAYFLY_VERSION}` },
      { text: ' · dsh 0.1.2-alpha.5', style: 'muted' },
    ])
  })
...
interface MountOptions {
  attach?: boolean
  /** Provide the four display services (fakeMayflyContext always does). */
  display?: boolean
...
describe('registerSessionCommands', () => {
  ...
  it('mounts the /status panel over the session facts and closes on Escape', async () => {
    ...
    expect(rows.some(row => row.includes(`Mayfly v${MAYFLY_VERSION}`))).toBe(true)
  ...
  it('opens the /version panel over the release lines and closes on Escape', async () => {
    ...
    const overlay = screen.overlays.at(-1)!
    const rows = plain((overlay.component as InfoPanel).render(80))
    expect(rows.some(row => row.includes(`v${MAYFLY_VERSION}`))).toBe(true)
  ...
  it('shows the same profile-local identity in /status and /version', async () => {
    ...
    expect(statusRows.some(row => row.includes(`Mayfly v${displayVersion}`))).toBe(true)
    await run(ctx, agent, '/version')
  ...
  it('opens the embedded /changelog panel and closes it', async () => {
    ...
    expect(rows.join('\n')).toContain('changelog')
    expect(rows.some(row => row.includes('Three-package distribution'))).toBe(true)
    overlay.component.handleInput?.('\x1b')
    expect(overlay.hidden).toBe(true)
  })
  ...
  it('guards /status, /context, and /version when the display services are missing', async () => {
    ...
    const status = await run(ctx, agent, '/status')
    expect(status).toEqual({ kind: 'error', text: 'status panel is unavailable: the Mayfly screen is not mounted' })
    const usage = await run(ctx, agent, '/context')
    expect(usage).toEqual({ kind: 'error', text: 'context panel is unavailable: the Mayfly screen is not mounted' })
    const version = await run(ctx, agent, '/version')
    expect(version).toEqual({ kind: 'error', text: 'version panel is unavailable: the Mayfly screen is not mounted' })
    const changelog = await run(ctx, agent, '/changelog')
    expect(changelog).toEqual({ kind: 'error', text: 'changelog panel is unavailable: the Mayfly screen is not mounted' })
  })
