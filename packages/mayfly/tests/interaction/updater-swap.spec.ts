...
import type { InteractiveChild, SpawnOutcome } from '../../src/interaction/updater/io.ts'
...
import {
  ...
  type SwapOutcome,
  type SwapProgress,
} from '../../src/interaction/updater/swap.ts'
...
function makeWorld(fromVersion = '0.1.0-rc.2') {
  const root = mkdtempTracked('mayfly-updater-swap-')
  const bundleDir = join(root, 'node_modules', '@ephemeral-ai', 'mayfly')
  mkdirSync(bundleDir, { recursive: true })
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
    "- id: mayfly-ui-provider",
    "  name: '@ephemeral-ai/mayfly-ui'",
    "- id: mayfly-core",
    "  name: '@ephemeral-ai/mayfly/core'",
    "- id: mayfly-theme-dark",
    "  name: '@ephemeral-ai/mayfly/theme-dark'",
...
/** The package managed by every install and rollback transaction. */
...
describe('updater/swap classifyInstallFailure', () => {
  it('translates each failure class', () => {
    ...
    expect(classifyInstallFailure('404 Not Found: @ephemeral-ai/mayfly@9.9.9')).toContain('does not serve this version')
...
describe('updater/swap patchEntrySpecs', () => {
  it('extracts every patch name row plus the declared dsh deps, deduplicated', () => {
    ...
    // A repeated Mayfly row proves the dedupe; the runtime dsh row and the
    // bundle manifest's declared deps join the sweep (D52④).
    const patch = join(world.root, 'node_modules', '@ephemeral-ai', 'mayfly', 'cordis.patch.yml')
    writeFileSync(patch, `${readFileSync(patch, 'utf8')}- id: mayfly-core-again\n  name: '@ephemeral-ai/mayfly/core'\n`)
...
describe('updater/swap importSweepSmoke', () => {
  it('passes on a zero exit and fails otherwise, logging the output', async () => {
    ...
    const log = readFileSync(join(world.root, '.mayfly-update-backup', 'update.log'), 'utf8')
...
describe('updater/swap performSwap', () => {
  ...
  it('succeeds end to end: snapshot, install, verify, both smokes', async () => {
    ...
    expect(install?.args).toEqual(['plugin', '--profile', 'mayfly', 'add', '@ephemeral-ai/mayfly@0.1.0-rc.3'])
    // The snapshot recorded the intent with the files it preserved.
    const snapshot = readFileSync(join(world.root, '.mayfly-update-backup', 'manifest.json'), 'utf8')
    ...
    expect(existsSync(join(world.root, '.mayfly-update-backup', 'pending.json'))).toBe(false)
  ...
  it('classifies a cooldown install failure and rolls the from-set back', async () => {
    ...
    expect(rollback?.args.slice(0, 4)).toEqual(['plugin', '--profile', 'mayfly', 'add'])
    // Rollback installs only the main Mayfly package.
    for (const name of RC2_NAMES) {
      expect(rollback?.args).toContain(`${name}@0.1.0-rc.2`)
    }
    expect(rollback?.args).not.toContain('@ephemeral-ai/mayfly-ui@0.1.0-rc.2')
  ...
  it('rolls back the main package without release-set metadata', async () => {
    const world = makeWorld()
    world.onInstall(specs => {
  ...
  it('installs the main package in one transaction on a downgrade', async () => {
    ...
    expect(outcome.kind).toBe('success')
    const install = world.spawns.find(call => call.args[0] === 'plugin')
    expect(install?.args.slice(0, 4)).toEqual(['plugin', '--profile', 'mayfly', 'add'])
    for (const name of RC2_NAMES) {
  ...
  it('refuses to roll back onto a pre-floor version and hands the recipe', async () => {
    const world = makeWorld('0.1.0-alpha.0')
    world.onInstall(() => fail('ERR_PNPM anything'))
    const outcome = await performSwap(swapInput(world, { fromVersion: '0.1.0-alpha.0' }))
    expect(outcome.kind).toBe('failed-no-rollback')
    expect(outcome.message).toContain('repair:')
    expect(outcome.message).toContain('0.1.0-alpha.1')
  ...
  it('reports an incomplete rollback when the reinstall fails', async () => {
    ...
    const pending = JSON.parse(readFileSync(join(world.root, '.mayfly-update-backup', 'pending.json'), 'utf8')) as Record<string, unknown>
  ...
  it('reports an incomplete rollback when the rolled-back tree is not one version', async () => {
    ...
    world.onInstall(specs => {
      ...
      // The rollback reinstall leaves the main package at the wrong version.
      writeFileSync(
        join(world.root, 'node_modules', '@ephemeral-ai', 'mayfly', 'package.json'),
        JSON.stringify({ name: '@ephemeral-ai/mayfly', version: '0.1.0-rc.3' }),
      )
      return ok()
    })
    const outcome = await performSwap(swapInput(world))
    expect(outcome.kind).toBe('rollback-incomplete')
    expect(outcome.message).toContain('rolled back but the package has the wrong version')
    const log = readFileSync(join(world.root, '.mayfly-update-backup', 'update.log'), 'utf8')
...
describe('updater/swap import sweep real execution', () => {
  ...
  function fixture(setup: {
    ...
    const root = mkdtempTracked('mayfly-updater-sweep-')
    const bundleDir = join(root, 'node_modules', '@ephemeral-ai', 'mayfly')
    mkdirSync(bundleDir, { recursive: true })
  ...
  it('passes over resolvable Mayfly packages and declared dsh deps', async () => {
    const root = fixture({
      declared: { '@deepseek-ai/dsh-fixture': '1.0.0' },
      rows: ['@ephemeral-ai/mayfly-dummy', '@deepseek-ai/dsh-fixture'],
      packages: {
        '@ephemeral-ai/mayfly-dummy': 'export const ok = 1\n',
  ...
  it('fails on a missing DECLARED dsh dep — the exemption is narrowed', async () => {
    const root = fixture({
      ...
      rows: ['@ephemeral-ai/mayfly-dummy', '@deepseek-ai/dsh-missing'],
      packages: { '@ephemeral-ai/mayfly-dummy': 'export const ok = 1\n' },
    })
    const outcome = await sweep(root)
  ...
  it('still exempts a missing UNDECLARED @deepseek-ai target (the host plane)', async () => {
    const root = fixture({
      rows: ['@ephemeral-ai/mayfly-dummy'],
      packages: { '@ephemeral-ai/mayfly-dummy': "import '@deepseek-ai/dsh-undeclared'\n" },
    })
    const outcome = await sweep(root)
    expect(outcome.code).toBe(0)
  })
