...
import { buildSkillsSections } from '../../src/interaction/skills-command.ts'
...
describe('registerSkillsCommand', () => {
  ...
  it('guards with an error when the Mayfly screen is not mounted', async () => {
    const { ctx, agent } = await mount({ display: false, skills: [skill('deploy-check')] })
    await expect(run(ctx, agent)).resolves.toEqual({
      kind: 'error',
      text: 'skills panel is unavailable: the Mayfly screen is not mounted',
    })
  })
})
