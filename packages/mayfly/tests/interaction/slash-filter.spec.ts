 ...
 * match ahead. The FakeMayflyComponents `fuzzyMatch` scores contiguity (-2
 * per character found in place, -1 per gap), which makes the alias
 * comparisons deterministic.
 */

import { describe, expect, it } from 'vitest'
import { filterSlashCommands, slashCommandLabel } from '../../src/interaction/slash-filter.ts'
import { FakeMayflyComponents } from './fakes.ts'
