...
import { CanonicalSettingsController, registerSettingsCommand, SettingsNoticeController } from '../../src/interaction/settings-command.ts'
...
function mount(options: BenchOptions = {}) {
  const { ctx, screen, components, theme } = fakeMayflyContext()
...
function fullSections(): Record<string, Record<string, unknown>> {
  return {
    mayfly: {
      updateCheck: true, updateChannel: 'latest', theme: 'dark',
...
function findList(node: import('@ephemeral-ai/mayfly-ui').MayflyUiNode): Extract<import('@ephemeral-ai/mayfly-ui').MayflyUiNode, { kind: 'list' }> | undefined {
...
describe('/settings registration and guards', () => {
  ...
  it('errors when the display quartet is absent', async () => {
    ...
    expect(result).toEqual({ kind: 'error', text: 'settings panel is unavailable: the Mayfly screen is not mounted' })
  })
...
describe('/settings level one', () => {
  it('moves the focus background together with the selected namespace', async () => {
    ...
    expect(before.find(row => row.includes('Mayfly UI preferences'))).toContain('{')

    panel.handleInput(KEY.down)
    const after = panel.render(80)
    expect(after.find(row => row.includes('Mayfly UI preferences'))).not.toContain('{')
  ...
  it('omits namespaces the host did not register', async () => {
    const bench = mount({ sections: { mayfly: fullSections().mayfly! } })
    await bench.command.handler()
    const panel = l1(bench.screen)
    const labels: string[] = []
    ...
    expect(labels).toEqual(['mayfly', 'Open settings.yaml in $EDITOR'])
  })
...
describe('/settings level two', () => {
  ...
  it('lists the mayfly rows in order, with resolved values and the namespace frame', async () => {
    const bench = mount({ sections: fullSections() })
    await bench.command.handler()
    await openNamespace(bench, 'mayfly')
    ...
    expect(items.map(item => item.id)).toEqual([
      'mayfly.updateCheck',
      'mayfly.updateChannel',
      'mayfly.theme',
      'mayfly.collapseThinking',
      'mayfly.collapseToolCalls',
      'mayfly.windowTurns',
      'mayfly.recentStepsRetention',
      'mayfly.expandTurns',
      'mayfly.userFoldLines',
    ...
    expect(byId.get('mayfly.windowTurns')?.currentValue).toBe('15')
    // The blank editor command displays through its emptyDisplay token.
    expect(byId.get('mayfly.editorCommand')?.currentValue).toBe('auto')
    expect(byId.get('mayfly.editorCommand')?.values).toEqual(['auto'])
    // The frame carries the namespace title and contextual operation row.
    const frame = frameText(bench)
    expect(frame).toContain('settings › mayfly')
  ...
  it('merges off-preset and unresolved current values into the cycle', async () => {
    const bench = mount({
      sections: {
        mayfly: { theme: 'ocean', editorCommand: 42 },
    ...
    const mayfly = await itemsOf('mayfly')
    // A key absent from the resolved section falls back to the first preset.
    expect(mayfly.get('mayfly.updateCheck')?.currentValue).toBe('true')
    // A non-string raw on the editable row still stringifies for display.
    expect(mayfly.get('mayfly.editorCommand')?.currentValue).toBe('42')
...
describe('/settings locale', () => {
  ...
  it('reprojects an open list in place and keeps its cursor and form draft', async () => {
    ...
    changeSetting(bench, 'mayfly.editorCommand', '')
  ...
  it('keeps an open list stable when its namespace disappears during locale refresh', async () => {
    ...
    await openNamespace(bench, 'mayfly')
    const panel = l2(bench.screen)
    bench.settings.drop('mayfly')
    bench.locale!.setPreference('zh')
    await settle()
    expect(l2(bench.screen)).toBe(panel)
    expect(panel?.render(80).join('\n')).toContain('settings › mayfly')
  })
})
...
describe('/settings writes', () => {
  it('cycles a boolean through the list keys and writes the parsed patch with the revision', async () => {
    ...
    await settle()
    expect(bench.settings.writes).toEqual([{ ns: 'mayfly', patch: { updateCheck: false }, revision: 1 }])
    ...
    bench.ctx.emit('settings/document-updated', 'mayfly' as SettingsNamespace, 2)
    await settle()
  ...
  it('writes enum and number cycles as strings and numbers', async () => {
    ...
    await bench.command.handler()
    await openNamespace(bench, 'mayfly')
    changeSetting(bench, 'mayfly.theme', 'ocean')
    await settle()
    l2(bench.screen)!.handleInput(KEY.escape)
    await openNamespace(bench, 'shell')
    ...
    await settle()
    expect(bench.settings.writes).toEqual([
      { ns: 'mayfly', patch: { theme: 'ocean' }, revision: 1 },
  ...
  it('retries a stale revision once and then succeeds', async () => {
    ...
    await bench.command.handler()
    await openNamespace(bench, 'mayfly')
    changeSetting(bench, 'mayfly.theme', 'paper')
    await settle()
    expect(calls).toBe(2)
  ...
  it('flashes the error after the retry also conflicts and rolls the row back', async () => {
    ...
    expect(list.updates).toEqual([['mayfly.theme', 'dark']])
    expect(list.options.items.find(item => item.id === 'mayfly.theme')?.currentValue).toBe('dark')
    expect(bench.screen.renderRequests).toBe(1)
  })
  ...
  it('flashes a plain write failure without retrying and rolls the row back', async () => {
    ...
    await bench.command.handler()
    await openNamespace(bench, 'mayfly')
    changeSetting(bench, 'mayfly.updateCheck', 'false')
    await settle()
    ...
    expect(settingsPanels(bench)[0]?.updates).toEqual([['mayfly.updateCheck', 'true']])
  })
  ...
  it('skips the notice when the panel closed behind a pending write', async () => {
    ...
    await bench.command.handler()
    await openNamespace(bench, 'mayfly')
    changeSetting(bench, 'mayfly.theme', 'light')
    closeAll(bench)
    release()
    await settle()
    expect(bench.notices).toEqual([])
...
describe('/settings editable rows', () => {
  it('opens the free-form form on Enter and writes the submitted text', async () => {
    ...
    panel!.handleInput(KEY.enter)
    await settle()
    expect(bench.settings.writes).toEqual([{ ns: 'mayfly', patch: { editorCommand: 'vim' }, revision: 1 }])
    // The form path pushes the display itself (pi-tui's cycle never moved).
    expect(settingsPanels(bench)[0]?.updates).toEqual([['mayfly.editorCommand', 'vim']])
  ...
  it('prefills the stored command and unsets the key on an empty submission', async () => {
    const bench = mount({
      sections: { ...fullSections(), mayfly: { ...fullSections().mayfly, editorCommand: 'nano' } },
    })
    await bench.command.handler()
    await openNamespace(bench, 'mayfly')
    changeSetting(bench, 'mayfly.editorCommand', 'nano')
    const panel = form(bench.screen)!
    ...
    await settle()
    expect(bench.settings.writes).toEqual([
      { ns: 'mayfly', ops: [{ op: 'unset', path: ['editorCommand'] }], revision: 1 },
    ])
    expect(settingsPanels(bench)[0]?.updates).toEqual([['mayfly.editorCommand', 'auto']])
...
describe('/settings refresh', () => {
  ...
  it('pushes changed values into the open list without remounting', async () => {
    ...
    await bench.command.handler()
    await openNamespace(bench, 'mayfly')
    bench.settings.sections.mayfly!.theme = 'ocean'
    ...
    expect(settingsPanels(bench)).toHaveLength(1)
    expect(list.updates).toEqual([['mayfly.theme', 'ocean']])
    expect(list.snapshotItems().find(item => item.id === 'mayfly.theme')?.currentValue).toBe('ocean')
    expect(bench.screen.renderRequests).toBe(1)
    // lastKnown moved with the diff: a second emission diffs empty.
    bench.ctx.emit('settings/document-updated', 'mayfly' as SettingsNamespace, 3)
    await settle()
  ...
  it('remounts only when the row set changed', async () => {
    ...
    expect(l1CursorLabel(l1(bench.screen))).toBe('mayfly')
  })
  ...
  it('absorbs a same-rowset change at level one without touching the panel', async () => {
    ...
    expect(settingItems(bench)
      .find(item => item.id === 'mayfly.theme')?.currentValue).toBe('ocean')
  })
  ...
  it('skips the refresh when the registration disposer ran before an event', async () => {
    ...
    bench.ctx.emit('settings/document-updated', 'mayfly' as SettingsNamespace, 5)
    await settle()
  ...
  it('drops the rolled-back row with the mid-write rebuild', async () => {
    ...
    changeSetting(bench, 'mayfly.theme', 'paper')
    await settle()
    // The rebuild retired the open list (mayfly is gone): level one alone is
...
describe('/settings open-file', () => {
  ...
  it('notices when no editor is configured', async () => {
    vi.stubEnv('VISUAL', '')
    vi.stubEnv('EDITOR', '')
    const dir = mkdtempTracked('mayfly-settings-command-')
...
describe('SettingsNoticeController', () => {
  it('bridges focus and admits compiler events for the wrapped canonical node', () => {
    const { theme, components } = fakeMayflyContext()
    const inner = {
      focused: false,
