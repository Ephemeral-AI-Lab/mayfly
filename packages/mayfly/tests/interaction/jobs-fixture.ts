...
export async function jobsFixture(ctx: Context) {
  ...
  const start = (options: { owner?: Agent | null, label?: string, output?: string, finalOnly?: boolean } = {}) => {
    ...
    const id = registry.start({
      kind: 'bash',
      label: options.label ?? 'Example job',
      ...(owner === undefined ? {} : { owner: owner.id }),
      run: job => {
        handle = job
        if (options.finalOnly !== true && options.output !== undefined && options.output !== '') job.append(options.output)
        return { cancel, done: done.promise }
      },
    })
    return { id, cancel, append(text: string) { handle?.append(text) }, async finish(outcome: JobOutcome) { done.resolve(outcome); await flushRequests() } }
  }
