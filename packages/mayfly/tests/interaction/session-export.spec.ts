...
describe('buildExportMarkdown', () => {
  ...
  it('renders the full event-stream view: nothing folded, injections labeled, chunks skipped', () => {
    ...
    expect(markdown).toContain('# Mayfly Session Export (full)')
...
describe('registerExportCommands', () => {
  ...
  async function mount(options: MountOptions = {}): Promise<{
    ...
    if (persistence !== undefined) {
      ...
      const projection = options.projection ?? {
        snapshot: () => ({ values: {
          mayflyConversation: conversationProjectionDefinition.wire.view(
  ...
  it('exports to an explicit path and notices the item count', async () => {
    const root = mkdtempTracked('mayfly-export-explicit-')
    ...
    expect(readFileSync(target, 'utf8')).toContain('# Mayfly Session Export')
  ...
  it('exports every official projection entry shape through the shared read path', async () => {
    const root = mkdtempTracked('mayfly-export-projection-')
  ...
  it('exports the full event-stream view with the full mode keyword', async () => {
    const root = mkdtempTracked('mayfly-export-full-')
  ...
  it('exports to the session cwd under the default mayfly-export name', async () => {
    const root = mkdtempTracked('mayfly-export-default-')
    ...
    expect(result).toEqual({ kind: 'success' })
    const matches = readdirSync(root).filter(name => name.startsWith('mayfly-export-') && name.endsWith('.md'))
  ...
  it('falls back to the process cwd when the session header recorded none', async () => {
    const root = mkdtempTracked('mayfly-export-nocwd-')
  ...
  it('decodes packed chunk rows through the dsh storage decoder', async () => {
    ...
    const root = mkdtempTracked('mayfly-export-chunks-')
    const target = join(root, 'out.md')
  ...
  it('reports a write failure as an error result', async () => {
    const root = mkdtempTracked('mayfly-export-writefail-')
