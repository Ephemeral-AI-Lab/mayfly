/**
 * The session-title terminal mirror (`mayfly-terminal-title`): the
...
import * as terminalTitle from '../../src/interaction/terminal-title.ts'
...
describe('mayfly-terminal-title', () => {
  ...
  it('re-folds on session switches in both directions', async () => {
    ...
    expect(screen.titles).toEqual(['first', 'second', 'mayfly'])
  })
})
