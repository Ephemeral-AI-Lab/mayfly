...
import {
  ...
  probeDarwin,
  probeWindows,
} from '../../src/interaction/paste-image-native.ts'
import {
  ...
  readCopiedPaths,
  runTool,
} from '../../src/interaction/clipboard-probe.ts'
...
const fs = require('fs')
const stage = process.env.MAYFLY_PASTE_STAGE
...
function fakeBin(withSips = true): { bin: string, log: string, sipsLog: string } {
  const bin = mkdtempTracked('mayfly-native-bin-')
...
describe('escapeAppleScriptString', () => {
  it('leaves plain POSIX paths alone', () => {
    expect(escapeAppleScriptString('/var/folders/t/mayfly-paste-1/clipboard.png')).toBe('/var/folders/t/mayfly-paste-1/clipboard.png')
  })
...
describe('clipboard probe primitives', () => {
  ...
  it('runs a helper with the default timeout and reports copied-file refusal bounds', async () => {
    ...
    const dir = mkdtempTracked('mayfly-native-files-')
...
describe('probeDarwin', () => {
  ...
  it('admits a Finder furl batch with space and CJK names, order preserved', async () => {
    const { bin } = fakeBin()
    process.env.PATH = bin
    const dir = mkdtempTracked('mayfly-native-furl-')
  ...
  it('escapes the staging path embedded in the AppleScript literal', async () => {
    ...
    const hostile = join(mkdtempTracked('mayfly-native-hostile-'), 'q"uote\\back')
    mkdirSync(hostile)
    ...
    expect(open).toContain(`POSIX file "${escapedRoot}/mayfly-paste-`)
