/**
 * Tests for the `mayfly-attachments` plugin: magic-byte sniffing, the
...
import * as attachmentsPlugin from '../../src/interaction/attachments.ts'
import { FilesystemAttachmentStore, sniffImageMediaType } from '../../src/interaction/attachments.ts'
...
describe('FilesystemAttachmentStore', () => {
  ...
  beforeEach(() => {
    root = mkdtempTracked('mayfly-attachments-')
    process.env.DSH_MAYFLY_ATTACHMENT_DIR = root
    const mayfly = fakeMayflyContext()
    ctx = mayfly.ctx
  })

  afterEach(async () => {
    delete process.env.DSH_MAYFLY_ATTACHMENT_DIR
  ...
  it('resolves the root from DSH_HOME when the dir override is unset', async () => {
    delete process.env.DSH_MAYFLY_ATTACHMENT_DIR
    const home = mkdtempTracked('mayfly-attachments-home-')
  ...
  it('enforces the byte and pixel caps against this.imageLimits', async () => {
    ...
    const capped = new CappedStore(fakeMayflyContext().ctx, { maxImageBytes: 10, maxImagePixels: 0 })
    ...
    const cappedPixels = new CappedStore(fakeMayflyContext().ctx, { maxImagePixels: 0 })
    ...
    const cappedDimension = new CappedStore(fakeMayflyContext().ctx, { maxImageDimension: 0 })
  ...
  it('saveImage maps write failures to ATTACHMENT_WRITE_FAILED', async () => {
    ...
    process.env.DSH_MAYFLY_ATTACHMENT_DIR = join(blocker, 'sub')
    const store = await mount()
