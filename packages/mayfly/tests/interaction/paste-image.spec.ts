/**
 * Tests for the `mayfly-paste-image` plugin: the Ctrl-V key wrapper over the
...
describe('mayfly-paste-image plugin', () => {
  let root: string
  let editor: FakeMayflyEditor
  let notices: string[]
  ...
  beforeEach(() => {
    root = mkdtempTracked('mayfly-paste-image-')
  ...
  it('consumes the paste key and delegates every other key to the previous handler', async () => {
    ...
    const bare = new FakeMayflyEditor()
  ...
  it('reattaches on mayfly/input-editor-changed without stacking wrappers', async () => {
    await mount()
    const first = editor.onKey
    const second = new FakeMayflyEditor()
    ...
    const third = new FakeMayflyEditor()
  ...
  it('stays silent without a notice callback', async () => {
    await mount()
    const plain = new FakeMayflyEditor()
...
`
  ...
  it('reads the first admitted type the listing offers, in admitted order', async () => {
    ...
    const bin = mkdtempTracked('mayfly-paste-bin-')
  ...
  it('pastes an ordered, deduplicated URI-list batch with encoded file names', async () => {
    const files = mkdtempTracked('mayfly-paste-files-')
  ...
  it('supports GNOME copied-files and treats cut as a read-only paste', async () => {
    const file = join(mkdtempTracked('mayfly-paste-files-'), 'castle.png')
  ...
  it('prefers direct image bytes over a copied-file representation', async () => {
    const file = join(mkdtempTracked('mayfly-paste-files-'), 'other.gif')
  ...
  it('falls back from invalid direct bytes to a valid copied image file', async () => {
    const file = join(mkdtempTracked('mayfly-paste-files-'), 'fallback.gif')
    writeFileSync(file, GIF_1X1)
    const bin = mkdtempTracked('mayfly-paste-bin-')
    tool(bin, 'wl-paste', wlPasteFake('image/png\ntext/uri-list\n', {
      'image/png': shBytes(new Uint8Array([1, 2, 3])),
  ...
  it('lets the mayfly.pasteImageBackend user layer override the composition backend', { timeout: 90_000 }, async () => {
    const bin = mkdtempTracked('mayfly-paste-bin-')
    ...
    process.env.PATH = bin
    process.env.DISPLAY = ':1'
    // The fake settings service: only the mayfly descriptor's RAW user layer
    // the override sync reads (never the resolved value).
    let present = true
    let user: unknown
    ctx.provide('settings', {
      describe: () => present
        ? [{ ns: 'mayfly', schema: {}, value: {}, revision: 1, applies: 'live', user }]
        : [],
    } as never)
  ...
  it('notices the missing tools when neither is on PATH', async () => {
    const bin = mkdtempTracked('mayfly-paste-empty-')
    process.env.PATH = bin
    await mountDefault()
    await vi.waitFor(() => {
  ...
  it('skips a timed-out backend during cooldown and retries it after expiry', async () => {
    ...
    process.env.XDG_RUNTIME_DIR = '/tmp/mayfly-runtime-new'
    editor.handleInput(KEY.ctrlV)
    await vi.waitFor(() => {
      expect(editor.inserted).toHaveLength(1)
    })
