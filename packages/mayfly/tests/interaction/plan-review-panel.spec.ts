...
describe('shared plan review', () => {
  ...
  it('recognizes only a valid pair and does not guess from option positions', () => {
    ...
    expect(JSON.stringify(planReviewControls(question, choices, key => key))).toContain('1. Start implementation')
    expect(JSON.stringify(planReviewControls(question, choices, key => key))).toContain('3. Other')
