 ...
 * @module @ephemeral-ai/mayfly/interaction/tests/jobs
 */
...
describe('mayfly-jobs command', () => {
  ...
  it('opens, reads explicitly, kills only live jobs, and closes on Agent change', async () => {
    ...
    panel.handleInput('K')
    expect(rig.registry.service.kill).toHaveBeenCalledWith(JobId('bash-1'), rig.agent, expect.stringContaining('Mayfly'))
