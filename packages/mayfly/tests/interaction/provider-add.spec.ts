...
import { ENDPOINT_PROTOCOLS, deriveKeyRef, runProviderAdd, runProviderEdit } from '../../src/interaction/provider-add.ts'
import { CanonicalSelectController, type SelectRow } from '../../src/interaction/select-list.ts'
...
function mountWizard(catalog: Parameters<typeof wizardLlm>[0], behavior: {
  ...
  const { ctx, screen, keymap, theme, components } = fakeMayflyContext()
