...
import { clipboardToolsFor, copyTextToClipboard, setClipboardOsc52Emitter, setClipboardTextWriter } from '../../src/interaction/clipboard-write.ts'
...
describe('default clipboard text writer', () => {
  ...
  function fakeBin(opts: {
    ...
    const bin = mkdtempTracked('mayfly-clipboard-bin-')
  ...
  it('probes wl-copy then xclip through the real tools', async () => {
    const wlCapture = mkdtempTracked('mayfly-clipboard-wl-')
    const bin = fakeBin({
      wlCopy: captureScript(join(wlCapture, 'copied.txt')),
      xclip: captureScript(join(mkdtempTracked('mayfly-clipboard-xc-'), 'copied.txt')),
    })
    prependBin(bin)
  ...
  it('falls back to xclip when wl-copy fails with a message', async () => {
    const xcCapture = mkdtempTracked('mayfly-clipboard-xc-')
    const bin = fakeBin({
      wlCopy: '#!/bin/sh\n echo "wayland gone" >&2\n exit 3\n',
  ...
  it('rejects with "not installed" when no tool exists on PATH', async () => {
    const empty = mkdtempTracked('mayfly-clipboard-empty-')
...
describe('copyTextToClipboard with an injected writer', () => {
  ...
  it('routes through the injected writer and restores the default', async () => {
    ...
    setClipboardTextWriter(undefined)
    const empty = mkdtempTracked('mayfly-clipboard-restore-')
    process.env.PATH = empty
