 ...
 * @module @ephemeral-ai/mayfly/interaction/tests/attach-view
 */
...
import {
  ...
  formatAttachElapsed,
  mountChildAttach,
  type MayflyChildAttachTarget,
} from '../../src/interaction/attach-view.ts'
...
function conversation(text: string, seq = 2): unknown {
  return {
    entries: [
      { id: 'u1', seq: seq - 1, updatedSeq: seq - 1, turn: 1, kind: 'user', text: 'hello child', images: [] },
      { id: 'a1', seq, updatedSeq: seq, turn: 1, kind: 'assistant', step: 0, text, streaming: false },
    ],
    streaming: false,
  }
}
...
function longConversation(turns: number): unknown {
  ...
  for (let index = 0; index < turns; index += 1) {
    entries.push(
      { id: `u${String(index)}`, seq: index * 2 + 1, updatedSeq: index * 2 + 1, turn: index + 1, kind: 'user', text: `line ${String(index)}`, images: [] },
      { id: `a${String(index)}`, seq: index * 2 + 2, updatedSeq: index * 2 + 2, turn: index + 1, kind: 'assistant', step: 0, text: `reply ${String(index)}`, streaming: false },
    )
  }
...
function toolConversation(): unknown {
  return {
    entries: [{
      id: 'tool-1', seq: 1, updatedSeq: 1, turn: 1, kind: 'tool', step: 0, callId: 'call-1',
...
async function mount(options: {
  readonly target?: MayflyChildAttachTarget
  ...
  ctx.provide('sessionProjections', {
    snapshot: () => ({
      ...
      values: options.snapshot?.values ?? { mayflyConversation: conversation('child reply') },
    }),
  ...
  ctx.provide('subagents', {
    ...
    prompt: async (request: { readonly content: readonly { readonly text?: string }[] }) => {
      followups.push(String(request.content[0]?.text))
      ...
      observeSession: async () => {
        ...
        return {
          ...(options.emptyObservation ? {} : {
            projections: {
              asOfSeq: options.snapshot?.asOfSeq ?? 2,
              values: options.snapshot?.values ?? { mayflyConversation: conversation('cold reply') },
            },
          }),
...
describe('ChildAttachView', () => {
  ...
  it('seeds a live child and renders stable framed rows at every supported width', async () => {
    const rig = await mount({ snapshot: { values: {
      mayflyConversation: conversation('initial reply'),
      mayflyConversationFacts: { active: false, epochTokens: 512 },
  ...
  it('degrades one-shot children to read-only while retaining scroll and interrupt', async () => {
    const rig = await mount({ target: { id: 'child', mode: 'one-shot' }, snapshot: { values: { mayflyConversation: longConversation(12) } } })
    rig.view.open()
  ...
  it('follows fresh projection values without letting a stale key rewind state', async () => {
    ...
    const rig = await mount({
      ...
      snapshot: { asOfSeq: 2, values: {
        mayflyConversation: conversation('stale cut'),
        mayflyConversationFacts: { epochTokens: 512 },
      } },
    })
    rig.view.open()
    rig.push(rig.child, 'mayflyConversation', conversation('fresh push', 5), 5)
    rig.push({ id: SessionId('other') } as Session, 'mayflyConversation', conversation('foreign'), 6)
    ...
    rig.push(rig.child, 'mayflyConversation', conversation('stale push', 3), 3)
    rig.push(rig.child, 'mayflyConversation', { entries: 'bad' }, 7)
    rig.push(rig.child, 'mayflyConversationFacts', { active: 'yes', epochTokens: 'many' }, 8)
    rig.push(rig.child, 'subagentTiming', null, 9)
    expect(plain(rig.view.render(80)).join('\n')).not.toContain('stale push')
    rig.push(rig.child, 'mayflyConversationFacts', { active: true, epochTokens: 4_096 }, 10)
    expect(plain(rig.view.render(80))[0]).toContain('● running · 4k tok')
    rig.view.dispose()
    rig.push(rig.child, 'mayflyConversation', conversation('late'), 20)
  })

  it('ticks only while running and can re-arm after settlement', async () => {
    vi.useFakeTimers()
    const rig = await mount({ snapshot: { values: {
      mayflyConversation: conversation('child reply'),
  ...
  it('scrolls by line and page, clamps both bounds, and preserves a parked viewport', async () => {
    const rig = await mount({ snapshot: { asOfSeq: 24, values: { mayflyConversation: longConversation(12) } } })
    rig.view.open()
    await vi.waitFor(() => expect(plain(rig.view.render(40)).join('\n')).toContain('reply 11'))
    ...
    rig.push(rig.child, 'mayflyConversation', longConversation(13), 26)
...
describe('mountChildAttach', () => {
  it('returns absent without a display and otherwise restores the editor slot on close', async () => {
    ...
    const base = await mount({
      ...
      snapshot: { values: { mayflyConversation: toolConversation() } },
    })
    const theme = new FakeTheme()
    base.ctx.provide('mayflyScreen', base.screen as never)
    base.ctx.provide('mayflyTheme', theme as never)
    base.ctx.provide('mayflyKeymap', {} as never)
    base.ctx.provide('mayflyComponents', new FakeMayflyComponents() as never)
    ...
    new PromptEditorController(base.ctx)
    new EditorPanelController(base.ctx)
    base.ctx.mayflyEditorPanels.setHost({ mount: component => base.screen.mountDialogPanel(component) })
    const closed = vi.fn()
    ...
    noResident.ctx.provide('mayflyScreen', noResident.screen as never)
    noResident.ctx.provide('mayflyTheme', theme as never)
    noResident.ctx.provide('mayflyKeymap', {} as never)
    noResident.ctx.provide('mayflyComponents', new FakeMayflyComponents() as never)
    ...
    new PromptEditorController(noResident.ctx)
    new EditorPanelController(noResident.ctx)
    noResident.ctx.mayflyEditorPanels.setHost({ mount: component => noResident.screen.mountDialogPanel(component) })
