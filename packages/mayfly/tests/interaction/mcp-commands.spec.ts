...
import {
  ...
  emptyMcpSections,
} from '../../src/interaction/mcp-commands.ts'
import type { McpCatalog, McpServerView } from '../../src/interaction/mcp-servers.ts'
import { FIBER_ACTIVE, FIBER_FAILED, MCP_CLIENT_MODULE } from '../../src/interaction/mcp-servers.ts'
...
describe('buildConfigSections', () => {
  it('renders status, counts, connection facts, and the resolved policy', () => {
    ...
    expect(flat.some(line => line.includes('Ephemeral-AI-Lab/mayfly/blob/main'))).toBe(true)
...
describe('emptyMcpSections', () => {
  it('guides to the profile patch, noting orphaned tools when present', () => {
    ...
    expect(flat[1]!.segments[0]!.text).toContain('Ephemeral-AI-Lab/mayfly/blob/main')
...
describe('registerMcpCommands', () => {
  ...
  it('mounts the guidance panel for an empty catalog, noting orphan tools', async () => {
    ...
    expect(rows.some(row => row.includes('Ephemeral-AI-Lab/mayfly/blob/main'))).toBe(true)
  ...
  it('guards the unmounted screen and the missing loader', async () => {
    ...
    expect(await run(bare.ctx, bare.agent, '/mcp')).toMatchObject({
      kind: 'error',
      text: 'mcp panel is unavailable: the Mayfly screen is not mounted',
    })
