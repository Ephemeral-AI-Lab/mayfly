...
describe('resolveExternalEditorCommand', () => {
  ...
  it('prefers a configured mayfly.editorCommand over $VISUAL/$EDITOR', async () => {
    ...
    await vi.waitFor(() => {
      expect(resolveExternalEditorCommand(
        ...
        settingsPlugin.currentMayflySettings(ctx).editorCommand,
      )).toBe('my-editor --wait')
    })
  })
})
...
describe('quoteShellArg', () => {
  it('single-quotes plain POSIX arguments', () => {
    expect(quoteShellArg('/tmp/mayfly-edit-x/prompt.md', 'linux')).toBe(`'/tmp/mayfly-edit-x/prompt.md'`)
  })
...
describe('default launcher', () => {
  ...
  function fakeEditorBin(name: string, script: string): string {
    const bin = mkdtempTracked('mayfly-editor-bin-')
  ...
  it('seeds the temp file, reads the edit back, and cleans up', async () => {
    const bin = fakeEditorBin('mayfly-fake-editor', '#!/bin/sh\necho ", edited" >> "$1"\nexit 0\n')
    prependBin(bin)
    await expect(runExternalEditor('seed text', 'mayfly-fake-editor')).resolves.toBe('seed text, edited\n')
  })

  it('resolves undefined on a nonzero exit — the :cq semantics', async () => {
    const bin = fakeEditorBin('mayfly-fake-editor', '#!/bin/sh\nexit 1\n')
    prependBin(bin)
    await expect(runExternalEditor('draft stays', 'mayfly-fake-editor')).resolves.toBeUndefined()
  })
  ...
  it('resolves undefined when the shell cannot find the editor command', async () => {
    ...
    prependBin(mkdtempTracked('mayfly-editor-bin-empty-'))
    await expect(runExternalEditor('draft stays', 'mayfly-missing-editor-x')).resolves.toBeUndefined()
  })
  ...
  it('treats a signal death as success and reads the seed back unchanged', async () => {
    ...
    const bin = fakeEditorBin('mayfly-fake-editor', '#!/bin/sh\nkill -9 $PPID\nexit 0\n')
    prependBin(bin)
    await expect(runExternalEditor('unchanged seed', 'mayfly-fake-editor')).resolves.toBe('unchanged seed')
  })
})
...
describe('launcher hook', () => {
  ...
  it('routes runExternalEditor through the injected launcher and restores on undefined', async () => {
    ...
    setExternalEditorLauncher(undefined)
    await expect(runExternalEditor('x', 'mayfly-missing-editor-x')).resolves.toBeUndefined()
  })
})
