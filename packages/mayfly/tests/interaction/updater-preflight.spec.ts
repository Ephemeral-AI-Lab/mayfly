...
import type { Packument } from '../../src/interaction/updater/registry.ts'
import { normalizePackument } from '../../src/interaction/updater/registry.ts'
import type { ProfileFacts } from '../../src/interaction/updater/profile.ts'
import {
  checkCooldown,
  checkDowngrade,
  checkHostLine,
  ...
  resolveOffer,
  runPreflight,
} from '../../src/interaction/updater/preflight.ts'
...
describe('updater/preflight checkLinkPollution', () => {
  ...
  it('blocks a link/file mix with the repair recipe', () => {
    const facts: ProfileFacts = { ...healthyFacts(), linked: ['@ephemeral-ai/mayfly'] }
    const verdict = checkLinkPollution(facts)
    expect(verdict.blocking).toBe(true)
    expect(verdict.message).toContain('@ephemeral-ai/mayfly')
    expect(verdict.message).toContain(repairRecipe(['<the names above>'], '<version>'))
  })
})

describe('updater/preflight checkSetConsistency', () => {
  it('passes when the Mayfly package is installed', () => {
    expect(checkSetConsistency(healthyFacts(), '0.1.0-rc.2', RC2_NAMES).blocking).toBe(false)
  })

  it('ignores versions of unrelated scoped packages', () => {
    const facts: ProfileFacts = {
      ...healthyFacts(),
      installed: { ...healthyFacts().installed, '@ephemeral-ai/other': '9.9.9' },
    }
    const verdict = checkSetConsistency(facts, '0.1.0-rc.2', RC2_NAMES)
    expect(verdict.blocking).toBe(false)
  })

  it('ignores an absent dependency record outside the managed package set', () => {
    const facts: ProfileFacts = { ...healthyFacts(), installed: { ...healthyFacts().installed, '@ephemeral-ai/mayfly-ui': undefined } }
    expect(checkSetConsistency(facts, '0.1.0-rc.2', RC2_NAMES).blocking).toBe(false)
  })

  it('blocks when nothing @ephemeral-ai is installed, with the target-set recipe', () => {
    const facts: ProfileFacts = { ...healthyFacts(), installed: {} }
    const verdict = checkSetConsistency(facts, '0.1.0-rc.2', RC2_NAMES)
    expect(verdict.blocking).toBe(true)
    expect(verdict.message).toContain('@ephemeral-ai/mayfly package is not installed')
    expect(verdict.message).toContain(`@ephemeral-ai/mayfly@<version>`)
  })
})
...
describe('updater/preflight checkVersionFloor', () => {
  it('passes the floor and everything above, blocks below it', () => {
    expect(checkVersionFloor('0.1.0-alpha.1').blocking).toBe(false)
    expect(checkVersionFloor('0.1.0-alpha.2').blocking).toBe(false)
    const verdict = checkVersionFloor('0.1.0-alpha.0')
    expect(verdict.blocking).toBe(true)
    expect(verdict.message).toContain("Mayfly's first release")
  })
})
...
describe('updater/preflight runPreflight', () => {
  ...
  it('carries the first blocking verdict for the caller', () => {
    const verdicts = runPreflight({
      facts: { ...healthyFacts(), linked: ['@ephemeral-ai/mayfly'] },
      packageNames: RC2_NAMES,
      currentVersion: '0.1.0-rc.2',
      target: '0.1.0-rc.3',
      packument: PACKUMENT,
...
describe('updater/preflight checkDowngrade', () => {
  it('warns without blocking when the target is older than the installed version', () => {
    ...
    expect(verdict.blocking).toBe(false)
    expect(verdict.message).toContain('downgrade reinstalls @ephemeral-ai/mayfly')
...
describe('updater/preflight resolveOffer', () => {
  ...
  it('refuses to offer a target below the floor', () => {
    const oldTag = normalizePackument({
      'dist-tags': { rc: '0.1.0-alpha.0' },
      versions: { '0.1.0-alpha.0': {} },
      time: {},
    })!
    expect(resolveOffer(oldTag, 'rc', '0.0.9')).toEqual({ kind: 'target-below-floor', target: '0.1.0-alpha.0' })
  })
})
