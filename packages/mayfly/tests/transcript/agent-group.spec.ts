...
import { AgentGroupComponent, setAgentGroupTimers, type AgentLiveLookup, type AgentMemberLive } from '../../src/transcript/agent-group.ts'
...
describe('AgentGroupComponent', () => {
  it('renders the running header and tree while members run', () => {
    ...
    const group = new AgentGroupComponent(agentMember(), tagged(), fakeMayflyComponents())
  ...
  it('renders the mixed breakdown header', () => {
    ...
    settle(first, 90_000)
    const group = new AgentGroupComponent(first, tagged(), fakeMayflyComponents())
    group.attach(agentMember({ seq: 2, callId: 'c2', parsedArguments: { description: 'Map docs', prompt: 'p' } }))
  ...
  it('falls back through the description chain', () => {
    ...
    const group = new AgentGroupComponent(withArgs, COLORS, fakeMayflyComponents())
  ...
  it('prefers a parsed name-like argument as the label', () => {
    ...
    const group = new AgentGroupComponent(named, tagged(), fakeMayflyComponents())
  ...
  it('ticks pending members forward and retires the timer when none remain', () => {
    const timers = fakeTimers()
    const group = new AgentGroupComponent(agentMember(), COLORS, fakeMayflyComponents())
    group.render(80)
  ...
  it('a tick over a still-pending member invalidates and nudges a redraw', () => {
    ...
    const group = new AgentGroupComponent(agentMember(), COLORS, fakeMayflyComponents(), () => { renders.push(1) })
    group.render(80)
  ...
  it('falls back to the bare Failed line when the error text is blank', () => {
    ...
    const group = new AgentGroupComponent(failed, COLORS, fakeMayflyComponents())
  ...
  it('counts failed members in the running breakdown', () => {
    ...
    const group = new AgentGroupComponent(failed, tagged(), fakeMayflyComponents())
    group.attach(agentMember({ seq: 2, callId: 'c2', parsedArguments: { description: 'Map docs', prompt: 'p' } }))
    expect(group.render(80)[1]).toBe(
  ...
  it('falls back to (no description) on settled members too', () => {
    ...
    const group = new AgentGroupComponent(done, COLORS, fakeMayflyComponents())
    group.attach(failed)
  ...
  it('starts no timer for a group that mounts already settled', () => {
    ...
    const group = new AgentGroupComponent(first, COLORS, fakeMayflyComponents())
    group.attach(second)
    group.render(80)
  ...
  describe('live overlay', () => {
    ...
    it('formats a non-finite live token count as zero', () => {
      ...
      timers.clock.now = T0 + 5_000
      const group = new AgentGroupComponent(
        agentMember(), COLORS, fakeMayflyComponents(), undefined,
    ...
    it('renders the waiting phase for an admitted-but-not-started child', () => {
      ...
      const group = new AgentGroupComponent(
        agentMember(), tagged(), fakeMayflyComponents(), undefined,
    ...
    it('sums tools and tokens in the settled tail and per-row stats', () => {
      ...
      settle(second, 45_000)
      const group = new AgentGroupComponent(
        first, tagged(), fakeMayflyComponents(), undefined,
        liveTable({
    ...
    it('mixes live and fold-only members: the tail sums the live side alone', () => {
      ...
      const group = new AgentGroupComponent(
        live, COLORS, fakeMayflyComponents(), undefined,
    ...
    it('fires the tick over live running members and stands down on their settle', () => {
      ...
      settle(acked, 35)
      const group = new AgentGroupComponent(
        acked, COLORS, fakeMayflyComponents(), () => { renders.push(1) }, liveTable(table),
      )
      group.render(80)
      expect(timers.intervalCount()).toBe(1)
      timers.fire()
    ...
    it('renders the settled tail with elapsed only when live members carry no tools', () => {
      ...
      settle(first, 30_000)
      const group = new AgentGroupComponent(
        first, COLORS, fakeMayflyComponents(), undefined,
