/**
 * `mayfly-pane-agents` plugin: the S33 subagent pane. Covers the zero-row
...
import * as paneAgents from '../../src/transcript/pane-agents.ts'
import {
  resetSeq,
  stepStart,
  toolCallEvent,
...
describe('mayfly-pane-agents plugin', () => {
  it('renders zero rows with no spawn calls', async () => {
    const { ctx, screen } = await boot()
    expect(screen.paneLines(80)).toEqual([])
    const entry = ctx.mayflyPanes.list().find(candidate => candidate.id === 'mayfly.pane.agents')
  ...
  it('holds a fresh waiting phase as running for one second, then reveals it', async () => {
    ...
    expect(rig.screen.paneLines(140).join('\n')).toContain('1s')
    expect(rig.ctx.mayflyPanes.list().find(entry => entry.id === 'mayfly.pane.agents')!.revision).toBeGreaterThan(revision)
    rig.ctx.emit('session/event', child, {
      type: 'tool/call', seq: 2, time: T0 + 2_000,
      data: { turn: 1, step: 1, callId: 't1', name: 'read', arguments: '{}' },
    })
