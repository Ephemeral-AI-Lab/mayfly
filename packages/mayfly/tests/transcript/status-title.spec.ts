/**
 * `mayfly-status-title` plugin: the folded session-title footer entry — the
...
import * as statusTitle from '../../src/transcript/status-title.ts'
...
describe('mayfly-status-title', () => {
  it('renders the folded title at priority 30, band 1 right cluster (the retired tips slot), muted tier', async () => {
    ...
    expect(harness.entry.id).toBe('mayfly.status.title')
