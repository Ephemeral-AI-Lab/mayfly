 ...
 * factory-backed `MayflyComponents`, width discipline, caching/invalidation,
...
import { DEFAULT_TRANSCRIPT_PRESENTATION } from '../../src/transcript/presentation-policy.ts'
import type {
...
function setup(): MayflyComponents {
  return fakeMayflyComponents()
}
...
describe('ToolCallComponent', () => {
  ...
  it('prefers the semantic chip over the raw line count when provided', () => {
    ...
    const dynamic = new ToolCallComponent(item, tagged(), setup(), undefined, '+1 −0')
    const first = dynamic.render(80)
    dynamic.setResultChip('+1 −0')
    expect(dynamic.render(80)).toBe(first)
    dynamic.setResultChip('+2 −1')
    expect(dynamic.render(80)[1]).toContain('+2 −1')
    dynamic.setResultChip(undefined)
    expect(dynamic.render(80)[1]).toContain('line')
  })
