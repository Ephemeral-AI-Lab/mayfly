...
import { apply, MayflyThemeService, name, OCEAN_COLORS } from '../../src/core/theme-ocean.ts'
...
describe('mayfly-theme-ocean plugin', () => {
  ...
  it('carries the teal logo sweep, frozen', async () => {
    const ctx = new Context()
    await ctx.plugin({ name, apply })
    const { logoGradient } = ctx.mayflyTheme.colors
    expect(logoGradient).toHaveLength(8)
    ...
    expect(logoGradient[7]!('hi')).toBe('\x1b[38;2;168;244;250mhi\x1b[39m')
  })
})
