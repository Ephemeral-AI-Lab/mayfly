 ...
 * listener restore), luminance classification, and the `mayflyTerminalInfo`
 * service snapshot.
 */
...
} from '../../src/core/terminal-info.ts'

interface FakeProc {
  proc: MayflyProbeProcess
  rawModes: boolean[]
  written: string[]
...
function fakeProc(options?: { rawMode?: boolean }): FakeProc {
  ...
  const stdin: MayflyProbeProcess['stdin'] = {
    on: (_event, listener) => {
...
describe('MayflyTerminalInfoService', () => {
  it('registers a frozen fact snapshot and unregisters with the fiber', async () => {
    const ctx = new Context()
    const fiber = ctx.plugin(MayflyTerminalInfoService, { background: 'light', kittyKeyboard: true })
    await fiber
    const info = ctx.mayflyTerminalInfo
    expect(info).toBeInstanceOf(MayflyTerminalInfoService)
