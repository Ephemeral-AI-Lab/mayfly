/**
 * `mayfly-theme-auto` plugin entry: initial palette selection from the probed
 * terminal background and provider swaps on `'mayfly/terminal-theme-changed'`.
 * `mayflyTerminalInfo` is faked with the real `MayflyTerminalInfoService`.
 */
...
import { apply, inject, name } from '../../src/core/theme-auto.ts'

/** Mount the fake terminal facts, then the auto theme plugin. */
async function mount(ctx: Context, background: 'dark' | 'light' | undefined) {
  await ctx.plugin(MayflyTerminalInfoService, { background, kittyKeyboard: false })
  const fiber = ctx.plugin({ name, inject, apply })
  await fiber
  return fiber
}

describe('mayfly-theme-auto plugin', () => {
  it('exposes its inject declaration and stable name', () => {
    expect(name).toBe('mayfly-theme-auto')
    expect(inject).toEqual(['mayflyTerminalInfo'])
  })
  ...
  it('mounts the light palette for a light terminal', async () => {
    ...
    expect(ctx.get('mayflyTheme')?.colors).toBe(LIGHT_COLORS)
  })
  ...
  it('swaps the provider when the terminal reports a scheme change', async () => {
    ...
    // Count dependent reloads: remounting `mayflyTheme` unloads and re-runs
    // every plugin injecting it through Cordis reload semantics.
    let reloads = 0
    await ctx.plugin({ name: 'theme-watcher', inject: ['mayflyTheme'], apply: () => {
      reloads++
    } })
    reloads = 0
    await ctx.parallel('mayfly/terminal-theme-changed', 'light')
    expect(ctx.get('mayflyTheme')?.colors).toBe(LIGHT_COLORS)
    expect(reloads).toBe(1)
    // A repeated report of the active scheme keeps the mounted provider.
    ctx.emit('mayfly/terminal-theme-changed', 'light')
    await Promise.resolve()
    expect(ctx.get('mayflyTheme')?.colors).toBe(LIGHT_COLORS)
    expect(reloads).toBe(1)
    // The chain supports swapping back and forth.
    await ctx.parallel('mayfly/terminal-theme-changed', 'dark')
    expect(ctx.get('mayflyTheme')?.colors).toBe(DARK_COLORS)
    await ctx.parallel('mayfly/terminal-theme-changed', 'light')
    expect(ctx.get('mayflyTheme')?.colors).toBe(LIGHT_COLORS)
