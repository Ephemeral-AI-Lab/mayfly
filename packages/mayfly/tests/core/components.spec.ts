...
const ROLES: (keyof MayflySemanticColors)[] = [
  'text',
  'textStrong',
  'muted',
  'textMuted',
...
function taggedTheme(): MayflyTheme {
...
function createService(tui: TuiMainScreen): MayflyComponentsService {
  return new MayflyComponentsService(new Context(), { theme: taggedTheme(), tui })
}
...
function createSgrService(tui: TuiMainScreen): MayflyComponentsService {
  return new MayflyComponentsService(new Context(), { theme: sgrTheme(), tui })
}

describe('MayflyComponentsService registration', () => {
  it('registers as ctx.mayflyComponents and unregisters when the fiber disposes', async () => {
    const { tui, stop } = bootTui()
    const ctx = new Context()
    const fiber = ctx.plugin(MayflyComponentsService, { theme: taggedTheme(), tui })
    await fiber
  ...
  it('owns the renderer SGR wrappers used by semantic nodes', () => {
    const { tui, stop } = bootTui()
    const components = createService(tui)
    expect(components.strong('strong')).toBe('\x1b[1mstrong\x1b[22m')
    expect(components.italic('italic')).toBe('\x1b[3mitalic\x1b[23m')
    expect(components.strike('strike')).toBe('\x1b[9mstrike\x1b[29m')
    stop()
  })
})
...
describe('createEditor', () => {
  it('keeps one real editor caret through layout and direct replay', () => {
    ...
    const components = new MayflyComponentsService(new Context(), { theme, tui })
    const result = compileMayflyUiNode(ui.form({ id: 'form', fields: [{ kind: 'input', id: 'name', label: 'Name', value: 'alpha' }] }), {
      components,
      colors: theme.colors,
  ...
  it('accepts and submits a slash completion on a single Enter (0.84.2 semantics)', async () => {
    ...
    const submits: string[] = []
    editor.onSubmit = text => submits.push(text)
    const completions: { item: MayflyAutocompleteItem, prefix: string }[] = []
    const provider: MayflyAutocompleteProvider = {
      triggerCharacters: ['@'],
      getSuggestions: () =>
        Promise.resolve({
  ...
  it('refreshes an open autocomplete list without changing the editor buffer', async () => {
    ...
    const suggestions = vi.fn(() => Promise.resolve({
      items: [{ value: 'quit', label: '/quit', description: 'Exit Mayfly' }],
      prefix: '/q',
    }))
  ...
  it('holds the real editor state before clear and commits exactly once', () => {
    ...
    const changes: string[] = []
    const submits: string[] = []
    let attempt: MayflyEditorSubmitAttempt | undefined
    editor.onChange = text => changes.push(text)
    editor.onSubmit = text => submits.push(text)
  ...
  it('aborts stale attempts on mutation, supersession, and barrier replacement', () => {
    const { tui, stop } = bootTui()
    const editor = createService(tui).createEditor()
    const attempts: MayflyEditorSubmitAttempt[] = []
    const submits: string[] = []
    editor.onSubmit = text => submits.push(text)
    ...
    let thrown: MayflyEditorSubmitAttempt | undefined
...
describe('createFileMentionProvider', () => {
  ...
  it('delegates to a spawned fd through the renderer pipeline', async () => {
    const { tui, stop } = bootTui()
    const components = createService(tui)
    const root = mkdtempTracked('mayfly-core-mention-')
    const bin = mkdtempTracked('mayfly-core-mention-bin-')
