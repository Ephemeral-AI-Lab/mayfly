/**
 * `ctx.mayflyTerminalInfo` service and the startup OSC 11 background probe.
 * The probe runs before the renderer takes over stdin (raw mode), so Mayfly
 ...
 *
 * @module @ephemeral-ai/mayfly/core/terminal-info
 */

import { Context, Service } from '@deepseek-ai/cordis'
import { parseOsc11BackgroundColor } from '@earendil-works/pi-tui'
import type { MayflyRgbColor, MayflyTerminalInfo } from './types.ts'

declare module '@deepseek-ai/cordis' {
  interface Context {
    mayflyTerminalInfo: MayflyTerminalInfoService
  }
}

/** The slice of the process the OSC 11 probe touches; tests inject a fake. */
export interface MayflyProbeProcess {
  stdin: {
...
export function probeTerminalBackground(
  proc: MayflyProbeProcess = process,
 ...
 */
export function backgroundFromRgb(rgb: MayflyRgbColor | undefined): 'dark' | 'light' | undefined {
...
/**
 * The `mayflyTerminalInfo` service: a frozen snapshot of probed terminal
 * facts. Unregistered automatically when the plugin's fiber unloads.
 */
export class MayflyTerminalInfoService extends Service implements MayflyTerminalInfo {
   ...
   */
  constructor(ctx: Context, info: MayflyTerminalInfo) {
    super(ctx, 'mayflyTerminalInfo')
