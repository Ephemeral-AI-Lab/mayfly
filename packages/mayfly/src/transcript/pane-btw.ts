/**
 * `mayfly-pane-btw` plugin: a side-question bottom pane plus the `/btw`
 ...
 * leave chrome, width, height allocation, and scrolling to Mayfly core.
 ...
 * `'mayfly/editor-connected-above'` (true on open, false on dismiss or
 * unload) and `mayfly-input` mirrors it onto the editor. While a dialog
 ...
 * re-asserts it on `'mayfly/editor-slot-swapped'` when the editor returns.
 * The pane renders no node while closed. The input plugin routes close and
 * continuation-submit commands through `'mayfly/btw-command'` while the editor
 * splice is connected.
 *
...
export const inject = ['mayflyPanes', 'commands', 'mayflyCurrentAgent', 'agents', 'agentDefaultModel', 'agentPresets', 'sessionProjections']
 ...
 */
function btwNode(state: BtwState): MayflyUiNode | null {
  ...
  for (const [index, turn] of state.turns.entries()) {
    ...
    children.push({ node: { kind: 'rich-text', spans: [{ text: `> ${turn.question}`, tone: 'accent', styles: ['strong'] }] } })
...
export function apply(ctx: Context): void {
  ...
  const ask = async (question: string): Promise<CommandResult> => {
    ...
    const initial = initialSnapshot.values['mayflyConversation']
    ...
    const updateStatus = (status: string): void => {
      ...
      turn.thinking = thinking
      if (!thinking) ctx.emit('mayfly/editor-connected-above', true, false)
      refreshDock()
    }
  ...
  // pane is open.
  ctx.on('mayfly/btw-command', (command, text) => {
  ...
  ctx.on('mayfly/editor-slot-swapped', (occupied) => {
    if (occupied) {
      ...
      if (state.open) ctx.emit('mayfly/editor-connected-above', false)
      return
    }
    if (!state.open) return
    const busy = state.turns.at(-1)?.thinking === true
    ctx.emit('mayfly/editor-connected-above', true, busy)
  })
