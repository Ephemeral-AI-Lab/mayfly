/**
 * `mayfly-pane-todo` plugin: a bottom pane rendering the attached session's
 * todo list. The list comes from the replay/live whole value exposed by
 * `mayflySessionFacts`, backed by the official `mayflyConversationFacts`
 ...
 *
 * @module @ephemeral-ai/mayfly/transcript/pane-todo
 */
...
export const inject = ['mayflyPanes', 'mayflyKeymap', 'mayflySessionFacts']

/** The global action toggling the todo panel's expansion (Ctrl-T). */
export const ACTION_TOGGLE_TODO = 'mayfly.todo.toggle'
...
function todoSpans(todo: TodoItem): readonly MayflyInlineSpan[] {
  switch (todo.status) {
    case 'completed':
      return [{ text: '  ✓ ', tone: 'success' }, { text: todo.content, tone: 'muted' }]
    case 'in_progress':
      return [{ text: '  ● ', tone: 'accent', styles: ['strong'] }, { text: todo.content }]
    case 'pending':
