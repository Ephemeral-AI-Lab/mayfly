/**
 * `mayfly-paste-image` plugin: Ctrl-V pastes a clipboard image into the input
 * editor as an attachment. The contextual `mayfly.image.paste` key action is
...
export const inject = ['attachments', 'mayflyKeymap', 'mayflyPromptEditor', 'mayflyPromptSubmissions', 'mayflyInteractionState']
 ...
 * `mayflyKeymap.matches`.
 */
export const ACTION_IMAGE_PASTE = 'mayfly.image.paste'
...
export function resetClipboardBackendCooldowns(ctx: Context): void {
  ctx.mayflyInteractionState.pasteImage.backendCooldowns.clear()
}
 ...
 * the handler `mayfly-input` installed.
...
function attach(ctx: Context, config: Config, shared: SharedEditor, isUnloaded: () => boolean): () => void {
  ...
  editor.onKey = (data) => {
    if (ctx.mayflyKeymap.matches(data, ACTION_IMAGE_PASTE)) {
 ...
 * shared editor whenever `mayfly-input` (re)mounts it; detach when it
...
export function apply(ctx: Context, config: Config): void {
  ...
  // The `mayfly.pasteImageBackend` user layer overrides the composition
  ...
  // re-read on every `mayfly` commit; a host without settings keeps the
  // composition value.
  const syncBackendOverride = (): void => {
    const descriptor = ctx.get('settings')?.describe().find(entry => String(entry.ns) === 'mayfly')
  ...
  ctx.effect(() => () => {
    ctx.mayflyInteractionState.pasteImage.backendOverride = undefined
  })
  ...
  ctx.on('mayfly/input-editor-changed', reattach)
  reattach()
}
