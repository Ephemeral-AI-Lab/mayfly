 ...
 * covers Mayfly's own `mayfly.*` (update check, default theme, folding
...
const ROWS: readonly SettingRow[] = [
  {
    ...
    description: 'Mayfly display language; system follows the operating system', kind: 'string',
    values: ['zh', 'en'], unsetValue: 'system',
    valueLabels: { system: 'Follow system', zh: '中文', en: 'English' },
  },
  {
    id: 'mayfly.updateCheck', ns: 'mayfly', key: 'updateCheck', label: 'Update check',
    description: 'boot update check on/off', kind: 'boolean', values: [true, false],
  },
  {
    id: 'mayfly.updateChannel', ns: 'mayfly', key: 'updateChannel', label: 'Update channel',
    description: 'dist-tag the boot check follows', kind: 'string', values: ['latest'],
  },
  {
    id: 'mayfly.theme', ns: 'mayfly', key: 'theme', label: 'Theme',
    description: 'default theme, applied at startup', kind: 'string',
    values: ['dark', 'light', 'ocean', 'paper', 'auto'],
  },
  {
    id: 'mayfly.collapseThinking', ns: 'mayfly', key: 'collapseThinking', label: 'Collapse thinking',
    description: 'thinking blocks start collapsed', kind: 'boolean', values: [true, false],
  },
  {
    id: 'mayfly.collapseToolCalls', ns: 'mayfly', key: 'collapseToolCalls', label: 'Collapse tool calls',
    description: 'tool output starts collapsed (ctrl+o toggles)', kind: 'boolean', values: [true, false],
  },
  {
    id: 'mayfly.windowTurns', ns: 'mayfly', key: 'windowTurns', label: 'Transcript window (turns)',
    description: 'completed turns kept mounted', kind: 'number', values: [5, 10, 15, 30, 50],
  },
  {
    id: 'mayfly.recentStepsRetention', ns: 'mayfly', key: 'recentStepsRetention', label: 'Recent steps kept',
    description: 'steps of a turn keeping their cards', kind: 'number', values: [10, 20, 30, 50, 100],
  },
  {
    id: 'mayfly.expandTurns', ns: 'mayfly', key: 'expandTurns', label: 'Ctrl-O range (turns)',
    description: 'turns the expansion toggle reaches back', kind: 'number', values: [1, 2, 3, 5, 10],
  },
  {
    id: 'mayfly.userFoldLines', ns: 'mayfly', key: 'userFoldLines', label: 'User fold lines',
    description: 'lines of a user message before it folds', kind: 'number', values: [5, 10, 20, 50],
  },
  {
    id: 'mayfly.userFoldChars', ns: 'mayfly', key: 'userFoldChars', label: 'User fold chars',
    description: 'characters of a user message before it folds', kind: 'number', values: [500, 1000, 2000, 5000],
  },
  {
    id: 'mayfly.editorCommand', ns: 'mayfly', key: 'editorCommand', label: 'External editor',
    description: 'ctrl+g editor command; empty follows $VISUAL/$EDITOR', kind: 'string',
    values: [], editable: true, emptyDisplay: 'auto',
  },
  {
    id: 'mayfly.pasteImageBackend', ns: 'mayfly', key: 'pasteImageBackend', label: 'Paste backend',
 ...
 */
function buildGroups(settings: SettingsProvider, dynamics: RowDynamics, t: MayflyTranslate): BuiltGroups {
 ...
 */
export class CanonicalSettingsController implements MayflyFocusable, CanonicalNodeSource {
 ...
 */
export class SettingsNoticeController implements MayflyFocusable {
  private readonly adapter: CanonicalPanelAdapter
  ...
  const dispose = ctx.commands.register({
    ...
    handler: async (): Promise<CommandResult> => {
      ...
      if (display === undefined) {
        return { kind: 'error', text: t('settings panel is unavailable: the Mayfly screen is not mounted') }
      }
