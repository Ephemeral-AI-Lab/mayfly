/**
 * `mayfly-attachments` plugin: provides the harness `attachments` service with
 ...
 * `mayflyComponents.imageDimensions` — only core may parse image bytes) and
 * persisted as `<attachmentId>.<ext>` files under the storage root:
 * `DSH_MAYFLY_ATTACHMENT_DIR`, else `$DSH_HOME/attachments`, else
 ...
 * `mayfly-paste-image` is the in-tree consumer. The admitted-type order and
 ...
 *
 * @module @ephemeral-ai/mayfly/interaction/attachments
 */
...
export const name = 'mayfly-attachments'
/** Services required before the store can decode image dimensions. */
export const inject = ['mayflyComponents']

/**
 * Admitted image media types in probe-preference order. Single source for
 * the store whitelist and for `mayfly-paste-image`, which negotiates
...
/**
 * File extension per admitted media type, shared with `mayfly-paste-image`
...
export class FilesystemAttachmentStore extends AttachmentStore {
  ...
  constructor(ctx: Context) {
    super(ctx)
    this.root = process.env.DSH_MAYFLY_ATTACHMENT_DIR
  ...
  private admission(input: SaveImageAttachment): { width: number, height: number } {
    ...
    const dimensions = this.ctx.mayflyComponents.imageDimensions(input.data)
