 ...
 * fallback inside mayfly-app; and `/version` — the banner constant and the live model as a
...
/**
 * The harness release line Mayfly builds against — independent of Mayfly's own
 * release version: the dsh-* dev pins stay on their own prerelease line
 * while {@link MAYFLY_VERSION} is Mayfly's first-release number. Guarded by
...
/**
 * Build the `/version` panel's sections (pure, for the spec): the Mayfly
 * release line and the harness line it builds against. The Mayfly number
 ...
 */
export function buildVersionSections(displayVersion = MAYFLY_VERSION): InfoSection[] {
  return [
    {
      heading: 'Version',
      rows: [
        { label: 'mayfly', segments: [{ text: `v${displayVersion}` }] },
...
export function buildChangelogSections(entries: readonly ChangelogEntry[]): InfoSection[] {
  return entries.map(entry => {
    ...
    return { heading: `v${entry.version}${entry.version === MAYFLY_VERSION ? ' · current' : ''}`, rows }
  })
}
 ...
 * @returns the sections in display order.
 */
export function buildStatusSections(input: StatusInput, displayVersion = MAYFLY_VERSION): InfoSection[] {
  return [
    {
      heading: 'Session',
      rows: [
 ...
 */
export function registerSessionCommands(ctx: Context, displayVersion = MAYFLY_VERSION): () => void {
  /**
  ...
  function showStatus(): CommandResult {
    ...
    if (display === undefined) {
      return { kind: 'error', text: 'status panel is unavailable: the Mayfly screen is not mounted' }
    }
  ...
  function showContext(): CommandResult {
    ...
    const display = displayServices(ctx)
    if (display === undefined) {
      return { kind: 'error', text: 'context panel is unavailable: the Mayfly screen is not mounted' }
    }
  ...
  /**
   * The `/version` handler: mount the read-only panel over the Mayfly and
  ...
  function showVersion(): CommandResult {
    const display = displayServices(ctx)
    if (display === undefined) {
      return { kind: 'error', text: 'version panel is unavailable: the Mayfly screen is not mounted' }
    }
    const restore = mountEditorReplacement(ctx, new InfoPanel({
      keymap: display.keymap,
      theme: display.theme,
  ...
  function showChangelog(): CommandResult {
    const display = displayServices(ctx)
    if (display === undefined) return { kind: 'error', text: 'changelog panel is unavailable: the Mayfly screen is not mounted' }
    const restore = mountEditorReplacement(ctx, new InfoPanel({
      keymap: display.keymap,
      theme: display.theme,
      components: display.components,
  ...
  const version = ctx.commands.register({
    name: 'version',
    description: 'Show the Mayfly and harness versions and the live model',
    handler: () => showVersion(),
  })
