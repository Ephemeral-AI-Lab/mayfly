...
export function registerPresetCommands(ctx: Context): () => void {
  ...
  async function openPresetPicker(): Promise<CommandResult> {
    ...
    if (display === undefined) {
      return { kind: 'error', text: 'preset picker is unavailable: the Mayfly screen is not mounted' }
    }
  ...
  const preset = ctx.commands.register({
    ...
    handler: invocation => {
      ...
      return roster.recompose(agent.ctx, id).then(preset => {
        if (ctx.mayflyCurrentAgent.current() !== agent) return { kind: 'error' as const, text: 'the active session changed before the preset switch completed' }
