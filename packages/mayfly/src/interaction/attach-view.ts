 ...
 *
 * @module @ephemeral-ai/mayfly/interaction/attach-view
 */
...
const ATTACH_PROJECTION_KEYS = ['mayflyConversation', 'mayflyConversationFacts', 'subagentTiming'] as const
...
export const ATTACH_CHROME = Object.freeze({
  ...
  guidanceBack: 'q back',
  unavailable: 'attach view is unavailable: the Mayfly screen is not mounted',
})
...
export interface MayflyChildAttachTarget {
  readonly id: string
...
export class ChildAttachView implements MayflyFocusable {
  focused = false
  ...
  private async seed(): Promise<ChildProjectionCut | undefined> {
    ...
    if (ctx.mayflyCurrentAgent.current() !== parent) return undefined
  ...
  private applyValue(key: string, value: unknown): void {
    ...
    if (key === 'mayflyConversationFacts') {
