 ...
 * `mayfly-paste-image` plugin: the reader-result contract every probe
 ...
 * display protocols (config-selectable through `mayfly-paste-image`) plus the
...
export async function withStagingDir<T>(fn: (stage: string) => Promise<T>): Promise<T> {
  const stage = await mkdtemp(join(tmpdir(), 'mayfly-paste-'))
  try {
    return await fn(stage)
  } finally {
    /* v8 ignore next -- best-effort cleanup; losing the race leaves one bounded mayfly-paste-* dir (D55) */
