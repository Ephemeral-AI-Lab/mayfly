 ...
 *
 * @module @ephemeral-ai/mayfly/interaction/updater/swap
 */
...
const MAYFLY_PACKAGE = '@ephemeral-ai/mayfly'
 ...
 * of the installed bundle's `cordis.patch.yml` — Mayfly packages and the
...
export function patchEntrySpecs(root: string): string[] {
  const patchText = updaterInternals.readTextFile(join(root, 'node_modules', '@ephemeral-ai', 'mayfly', 'cordis.patch.yml'))
  const specs: string[] = []
...
export function declaredRuntimeDeps(root: string): string[] {
  const text = updaterInternals.readTextFile(join(root, 'node_modules', '@ephemeral-ai', 'mayfly', 'package.json'))
...
export function importSweepScript(specs: readonly string[], declared: readonly string[]): string {
  return [
    ...
    "const manifest = realpathSync(join(process.cwd(), 'node_modules', '@ephemeral-ai', 'mayfly', 'package.json'));",
    ...
    // @ephemeral-ai package, a hashed chunk file — is exactly the D51 class
 ...
 * transaction, verify the main package, run both smoke layers, and on any
 * failure restore the snapshot and reinstall the old version. A
 * `pending.json` marker in the backup
...
/** Whether the installed main package has the expected version. */
function packageIsVersion(root: string, version: string): boolean {
  return readProfileFacts(root).installed[MAYFLY_PACKAGE] === version
}
...
async function rollback(input: SwapInput, reason: string, logPath: string, pendingPath: string): Promise<SwapOutcome> {
  ...
  if (!packageIsVersion(input.root, input.fromVersion)) {
    progress(input, 'rollback', 'fail')
