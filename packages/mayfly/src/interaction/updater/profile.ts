...
export const BACKUP_DIRNAME = '.mayfly-update-backup'
...
export function readProfileFacts(root: string): ProfileFacts {
      ...
      if (typeof parsed === 'object' && parsed !== null) {
        ...
        for (const key of ['dependencies', 'devDependencies'] as const) {
          ...
          for (const [name, spec] of Object.entries(block as Record<string, unknown>)) {
            ...
            if (name === '@ephemeral-ai/mayfly' && /^(link|file):/.test(spec)) linked.push(name)
          }
        }
      }
    } catch {
  ...
  const packageName = '@ephemeral-ai/mayfly'
  const installed: Record<string, string | undefined> = {
    [packageName]: readInstalledVersion(join(root, 'node_modules', packageName, 'package.json')),
  }
...
/**
 * Snapshot the profile's manifest files into `.mayfly-update-backup/`
