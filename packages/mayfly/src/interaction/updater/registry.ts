/**
 * Registry metadata for the updater (D52): the `@ephemeral-ai/mayfly` packument
 ...
 *
 * @module @ephemeral-ai/mayfly/interaction/updater/registry
 */
...
const REGISTRY_URL = 'https://registry.npmjs.org/@ephemeral-ai/mayfly'
...
async function npmView(): Promise<RegistryResult | undefined> {
  const outcome = await updaterInternals.spawnOnce('npm', ['view', MAYFLY_PACKAGE, '--json'], {
 ...
 * Mayfly UI remains a normal dependency of the main package, so the package
 * manager resolves it as part of the main package's dependency closure.
...
async function viewDependencies(version: string): Promise<Record<string, string> | undefined> {
  const outcome = await updaterInternals.spawnOnce('npm', ['view', `${MAYFLY_PACKAGE}@${version}`, 'dependencies', '--json'], {
    timeoutMs: VIEW_TIMEOUT_MS,
  })
