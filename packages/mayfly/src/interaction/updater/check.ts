/**
 * `mayfly-update-check` plugin (D52): the boot-time registry check. After
 ...
 * sent, and `mayfly.updateCheck: false` in `~/.dsh/settings.yaml` makes
 * Mayfly fully offline.
 *
 ...
 *
 * @module @ephemeral-ai/mayfly/interaction/updater/check
 */
...
/** The check's persisted state, under `$DSH_HOME/storages/mayfly-update/`. */
...
/** The update-specific view of the shared Mayfly settings namespace. */
export type UpdateSettings = Pick<MayflySettings, 'updateCheck' | 'updateChannel'>
...
export function updateCheckStatePath(): string {
  return join(dshHome(), 'storages', 'mayfly-update', 'state.json')
}
...
export async function runUpdateCheck(
  ...
  if (previous !== undefined && now - previous.lastCheckAt < CHECK_INTERVAL_MS) {
    ...
    if (offer !== undefined && compareVersions(offer.version, MAYFLY_VERSION) > 0) {
  ...
  const offer = resolveOffer(result.packument, settings().updateChannel, MAYFLY_VERSION)
  if (offer.kind !== 'offer') {
...
/**
 * Mount the boot check; `mayfly-settings` owns the single settings section.
 * @param ctx - plugin context.
 */
export function apply(ctx: Context): void {
  ...
  void runUpdateCheck(ctx, () => currentMayflySettings(ctx), () => unloaded).catch(() => {})
}
