/**
 * `mayfly-editor-plus` plugin: enhancement layer over the shared input editor
 * mounted by `mayfly-input` — the `prompt | bash` input mode with shell
 ...
 * over `mayfly-input`'s slash-context resolution while the mode is active),
 ...
 * this plugin after `mayfly-input` (which provides no service), so
 * attach/detach is driven by the `'mayfly/input-editor-changed'` event, which
 ...
 * dead context.
 *
 * @module @ephemeral-ai/mayfly/interaction/editor-plus
 */
  ...
  return {
    ...
    async getSuggestions(lines, cursorLine, cursorCol, options): Promise<MayflyAutocompleteSuggestions | null> {
      ...
      if (skillQuery !== null) {
        ...
        ).map((match): MayflyAutocompleteItem => {
      ...
      const items = filterSlashCommands(
        ctx.mayflyInteractionState.aliases.withCommandAliases(ctx.commands.list(agent).map(command => ({
          name: command.name,
 ...
 * preserving the handlers `mayfly-input` installed.
...
function attach(ctx: Context, shared: SharedEditor, isUnloaded: () => boolean): () => void {
  ...
  editor.onSubmit = (text) => {
    ...
    // stash mirror matches `mayfly-input`'s, so bash entries survive a
  ...
  editor.onKey = (data) => {
    // `mayfly-input`'s chain runs first: an open side-question pane or a
    ...
    if (mode === 'bash'
      ...
      && (ctx.mayflyKeymap.matches(data, ACTION_CANCEL) || ctx.mayflyKeymap.matches(data, ACTION_BACKSPACE))) {
      exitBash()
      return true
    }
    return false
...
/**
 * Attach to the shared editor whenever `mayfly-input` (re)mounts it; detach
 * when it unmounts or this fiber disposes.
 * @param ctx - plugin context.
 */
export function mountEditorPlus(ctx: Context, shared: SharedEditor, isUnloaded: () => boolean): () => void {
  const detach = attach(ctx, shared, isUnloaded)
