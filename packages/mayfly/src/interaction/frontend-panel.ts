 ...
 * the resulting public Mayfly UI node.
 *
 * @module @ephemeral-ai/mayfly/interaction/frontend-panel
 */
...
export class CanonicalDocumentController implements MayflyFocusable {
  ...
  private emptyNode(model: FrontendPanelDocument, group: string): MayflyUiNode {
  ...
  private itemDetailSpans(item: FrontendPanelItem, variants: readonly FrontendPanelVariant[]): readonly MayflyInlineSpan[] {
    const selected = this.selectedVariant(item)
    if (item.variantsFirst === true) {
      const spans: MayflyInlineSpan[] = variants.map((variant, index) => {
        const active = variant.id === selected?.id
        return {
          text: `${index === 0 ? '' : ' '}[${variant.label}]`,
          tone: active ? 'accent' : 'muted',
          ...(active ? { styles: ['strong'] as const } : {}),
        }
      })
      if (item.detail !== undefined) spans.push({ text: ` · ${item.detail}` })
      return spans
    }
    const spans: MayflyInlineSpan[] = item.detail ? [{ text: item.detail }] : []
...
function nodeText(node: MayflyUiNode | undefined): string | undefined {
