/**
 * `@`-mention completion support for `mayfly-editor-plus` — the Mayfly half of
 ...
 *
 * @module @ephemeral-ai/mayfly/interaction/file-mention
 */
 ...
 */
function toMentionItem(candidate: FsMentionCandidate): MayflyAutocompleteItem {
...
export async function listDirectoryMentions(
  ...
  if (signal.aborted) return null
  const items: MayflyAutocompleteItem[] = []
