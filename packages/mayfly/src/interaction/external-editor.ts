 ...
 * the editor command — the persisted `mayfly.editorCommand` setting wins,
 ...
 *
 * @module @ephemeral-ai/mayfly/interaction/external-editor
 */
...
/**
 * Resolve the external editor command: a non-blank `mayfly.editorCommand`
...
async function editInExternalEditor(initialText: string, command: string): Promise<string | undefined> {
  const dir = await mkdtemp(join(tmpdir(), 'mayfly-edit-'))
