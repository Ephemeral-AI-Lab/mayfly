 ...
 *
 * @module @ephemeral-ai/mayfly/app/retraction
 */
...
export interface MayflyRetractionService { tryRetract(messageId: string): boolean }
...
interface PendingRetraction extends OpenTurn {
  ...
  readonly lifecycle: MayflyTurnRetraction
}
...
export function installRetractionService(
  ...
  const service: MayflyRetractionService = {
    tryRetract(messageId) {
      ...
      const lifecycle: MayflyTurnRetraction = {
  ...
  ctx.provide('mayflyRetractions', service)
  ctx.effect(() => () => {
    pending = undefined
  })
  return service
